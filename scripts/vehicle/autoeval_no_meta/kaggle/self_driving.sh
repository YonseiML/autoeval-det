...
for i in `seq 0 0`;
  ...
  if [ -f "./res_iou_ac_area_filter/${TAG1}/${i}.json" ]; then
    ...
    python tools/test_test.py configs/${DETECTOR}/${DETECTOR}_${BACKBONE}_fpn_1x_coco_${TASK}.py ./checkpoints/${DETECTOR}_${BACKBONE}_${TASK}/epoch_36.pth --eval bbox --cfg-option data.test.img_prefix=./data/kaggle/${DATASET}/self_driving_cars_kaggle/images data.test.ann_file=${ANN_DIR}  --tag1 ${TAG1} --tag2 ${i} --task ${TASK} --detector ${DETECTOR} --backbone ${BACKBONE}
  fi
done
