...
from mmdet.core.post_processing.nms import batched_nms_with_iou
...
class BBoxHead(BaseModule):
    ...
    def get_bboxes(self,
        ...
        if cfg is None:
            ...
            det_bboxes, det_labels, inds, iou_values_all, iou_values_keep, init_boxes, scores_ori = multiclass_nms(bboxes, scores,
                                                    cfg.score_thr, cfg.nms,
                                                    cfg.max_per_img, return_inds=True)
            
            self.pre_nms_cls_score_value.append(scores_ori) 
            self.post_nms_cls_score_value.append(det_bboxes[:, -1])
            self.post_nms_det_bbox_value.append(det_bboxes[:, :4])
            self.iou_all_values.append(iou_values_all)
            self.iou_keep_values.append(iou_values_keep)
            self.init_boxes.append(init_boxes)
