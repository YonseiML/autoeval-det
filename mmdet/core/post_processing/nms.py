...
from mmdet.core.post_processing import ext_loader
...
def batched_nms_with_iou(boxes: Tensor,
                scores: Tensor,
                idxs: Tensor,
                nms_cfg: Optional[Dict],
                class_agnostic: bool = False) -> Tuple[Tensor, Tensor]:
    ...
    init_boxes = boxes[:, :4]
    iou_values_all = torchvision.ops.box_iou(boxes[:, :4], boxes[:, :4])
    ...
    iou_values_keep = torchvision.ops.box_iou(boxes[:, :4], boxes[:, :4])

    return boxes, keep, iou_values_all, iou_values_keep, init_boxes
