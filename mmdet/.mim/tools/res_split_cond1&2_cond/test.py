...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        adjacent_iou = iou_values_all[current_idx][iou_with_remaining]
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            # if (iou_mean.item() < 0.65) & (inter_over_pseudo_box.item() == 1):
                # print("###################")
                # print("iou_mean: ", torch.mean(adjacent_iou))
                up07 = adjacent_iou >= 0.7
                # print("ratio: ", up07.sum()/adjacent_iou.shape[0])
                if (up07.sum()/adjacent_iou.shape[0]).item() != 0:
            ...
            # if (iou_mean.item() < 0.7) & (iou_mean2.item() < 0.7) & (inter_over_pseudo_box.item() >= 2/3):
                ...
                # # pseudo_iou = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox.unsqueeze(0))
                # # print(pseudo_iou)
            # print(torch.mean(adjacent_iou))
            # up07 = adjacent_iou >= 0.7
            # print(up07.sum()/adjacent_iou.shape[0])
            pseudo_iou = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox.unsqueeze(0))
...
def main():
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_split_cond1_cond_iou/" + str(args.tag1)
