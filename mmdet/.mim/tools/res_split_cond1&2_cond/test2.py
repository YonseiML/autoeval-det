    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            # if (iou_mean.item() < 0.65) & (inter_over_pseudo_box.item() == 1):
                ...
                # pseudo_box_area = torchvision.ops.box_area(pseudo_bbox) # 원래
                ref_box_area = torchvision.ops.box_area(ref_bbox)
                adjacent_over_pseudo_box = intersection / ref_box_area[:, None]
                # print(adjacent_over_pseudo_box)
                area1 = adjacent_over_pseudo_box == 1
                # print(area1)
                # print(area1.sum())
                if area1.sum().item() >= 1:
...
def main():
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_split_cond1_cond/" + str(args.tag1)
