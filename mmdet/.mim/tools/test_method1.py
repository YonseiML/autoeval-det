...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        # if over_under.mean() > 0 :
        #     num += over_under.mean()/sorted_scores[i]
    ...
    # keep이 score_all에 적용하면 score_ex으로 나옴 
    # print("num", num)
    # exit()
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            # score_ex = torch.tensor(results_cls_per_img[img_idx])   # sort 되어있음  # sort 안되어있는거는 apply_iou_filter의 결과 이용하면 됨
            score_all = torch.tensor(results_cls_per_img_ori[img_idx])
            ...
            if len(iou_all) > 0 :
                ...
                iou_all_mean = torch.mean(torch.tensor(np.array(iou_all)), dim=-1)
                iou_ex_mean = torch.mean(torch.tensor(np.array(iou_ex)), dim=-1)
                iou_ex_double_mean = torch.mean(iou_ex_mean)
                score_ex_mean = torch.mean(score_ex)
                dot = score_all * iou_all_mean
                # print(dot)
                # print(score_ex_mean * iou_ex_double_mean)
                # exit()
                ou = torch.sum(dot < score_ex_mean * iou_ex_double_mean)
                ou2 = torch.sum(dot > score_ex_mean * iou_ex_double_mean)
                
                # print(len(score_ex))
                # print(ou/len(score_all))
                # exit()
                if ou > 0 :
                    over_under.append(ou/len(score_all))
                    print(ou/len(score_all))

                if ou2 > 0 :
                    measures.append(ou2/len(score_all))
                    print(ou2/len(score_all))
                # exit()
            # print(torch.mean(iou_all_mean))
            # print(torch.mean(iou_ex_mean))
            # print((torch.mean(score_all) - torch.mean(score_ex)))
            # print(torch.mean(iou_all_mean)-torch.mean(iou_ex_mean))
            # thres = (torch.mean(score_all) - torch.mean(score_ex))/(torch.mean(iou_all_mean)-torch.mean(iou_ex_mean))
            # thres = torch.mean(score_all)/torch.mean(score_ex)
            # print(thres)
            # print(torch.mean(iou_ex_mean)/torch.mean(iou_all_mean))
            # print(thres*(torch.mean(iou_ex_mean)/torch.mean(iou_all_mean)))
            # measures.append(thres*(torch.mean(iou_ex_mean)/torch.mean(iou_all_mean)))
            # exit()
            # if thres >= 0 :
            # exit()
            #     measures.append(1)
            # else :
            #     measures.append(0)
            # print((torch.mean(score_all) - torch.mean(score_ex))/(torch.mean(iou_all_mean)-torch.mean(iou_ex_mean)))
            # exit()
            ############ score filter
            # score_all = score_all[score_all>0.05]
            # keep_idx, under_over_num = apply_iou_filter(score_all, iou_all, 0.5)  # 주석 풀면 됨
            # score_ex_not_sort = score_all[keep_idx]
            # re, _ = torch.sort(score_ex_not_sort, descending=True)
            # print(re)
            # # print(score_ex_not_sort)
            # mask = np.array(keep_idx, dtype=bool)
            # print(mask)
            # iou_all = np.array(iou_all)
            # re = iou_all[mask, :][:, mask]   # 이거랑 iou_ex랑 다름 
            # # print(iou_all)
            # # print(keep_idx)
            # # print(re[0:3])
            # print(re[0])
            # exit()
            # # print(remove_idx)
            # # print(under_over_num)
            
            # print(remove_num)
            # print(under_over_num/remove_num)
            ##################
            ...
            #     over_under.append(0)
            # else:
            #     over_under.append(under_over_num/len(score_ex))
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_0909_3_modi/" + str(args.tag1)
            ...
            with open(NAME, "r", encoding="utf-8") as f:
                ...
                if data == {}:
                    data.update({0: [[metric['bbox_mAP'], metric['bbox_mAP_50'], metric['bbox_mAP_75'], metric['bbox_mAP_s'], metric['bbox_mAP_m'], metric['bbox_mAP_l']], least_cost, \
                        least_iou_cost_final,  result_over_under,result_measures, least_reg_cost_final, least_cls_cost_final, num_cls_large_k, mean_cls_large_k, results_cls_entropy_small_k]})
                else:
