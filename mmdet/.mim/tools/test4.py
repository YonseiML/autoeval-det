...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        # print(removed_box.shape)
        # exit()
        # agnostic_nms = (0.5 + 0.5 / (1 + np.exp(-10 * (remove_score - 0.5)))).sum()
        agnostic_nms = (0.5 + 0.5 / (1 + np.exp(-10 * (remove_score - 0.7)))).sum()
        adj_box_confup_num.append(agnostic_nms.item())
        if removed_box.numel() != 0:
            # print(remove_score.shape)
            remove_score_up = remove_score[remove_score>0.7]
            # print(remove_score_up.shape)
            # exit()
            agnostic_nms_up = (0.5 + 0.5 / (1 + np.exp(-10 * (remove_score_up - 0.7)))).sum()
            adj_box_confup_num_up.append(agnostic_nms_up.item())
        # else:
        #     adj_box_confup_num.append(0)
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            pre_iou = torchvision.ops.box_iou(predicted_bbox.unsqueeze(0) , removed_box)
            # model_score1 = abs(abs(predicted_score-remove_score)-(1-pre_iou.squeeze(0))) # # res_reg_dist_sig10_reg_noabs_agu3_2
            model_score1 = abs(predicted_score-remove_score)-(1-pre_iou.squeeze(0))  # res_reg_dist_sig10_reg_noabs_agu3
            # reg.append(((model_score>0.2).sum()/remove_score.shape[0]).item())
            model_score2 = abs(predicted_score-remove_score)-(1-pre_iou.squeeze(0))
            # print(torch.mean(model_score).item())
            # exit()
            # print(pre_iou.shape)
            # print(abs(predicted_score-remove_score).shape)
            # print(model_score1)
            # model_score3 = torch.mean(abs(predicted_score-remove_score)) - torch.mean(1-pre_iou.squeeze(0))
            # # print(model_score3)
            # # exit()
            reg.append(torch.mean(model_score1).item())
            # reg2.append(model_score3.item())
            # conf_d.append(abs(predicted_score-remove_score))
            # iou_d.append((1-pre_iou.squeeze(0)))
            reg2.append(torch.mean(model_score2).item())
    ...
    # conf_d  = torch.mean(torch.tensor(conf_d))
    # iou_d = torch.mean(torch.tensor(iou_d))
    # print(conf_d-iou_d)
    # exit()
    
    # return kept_indices, remove_bbox
    return keep, torch.tensor(pseudo_ious), torch.tensor(pseudo_ious2), torch.tensor(reg2), torch.tensor(adj_box_confup_num), torch.tensor(adj_box_confup_num_up)
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep, pseudo_ious, pseudo_ious2, reg2, adj_box_num, adj_box_num_up = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                # conf_up = conf > 0.5
                # conf_down = conf <= 0.5
                # under = (0.5 + 0.5 / (1 + np.exp(-10 * (score_all - 0.5)))).sum()
                # conf_adj = conf - 0.1*reg2
                # conf_up = conf_adj > 0.7
                # # conf_down = conf <= 0.7
                # # # under = (0.5 + 0.5 / (1 + np.exp(-10 * (score_all - 0.5)))).sum()
                # # under = (0.5 + 0.5 / (1 + np.exp(-10 * (score_all - 0.7)))).sum()
                # under = (0.5 + 0.5 / (1 + np.exp(-10 * (score_all[~keep] - 0.7)))).sum() #2
                # # # under = adj_box_num.sum() #1
                # num_box_ratio = adj_box_num[conf_up].sum()/under
                # # # print(num_box_ratio)
                # # # exit()
                # # if under.item() > 0 :
                # #     over_under_6.append(num_box_ratio.item())
                # # print(adj_box_num_up.sum()/adj_box_num.sum())
                # # exit()
                # over_under_6.append(adj_box_num_up.sum()/adj_box_num.sum())


                # conf_adj = conf - 0.5*reg2  # 그 전 02.18은 0.1이었음음
                # conf_up = conf_adj > 0.7
                # under = (0.5 + 0.5 / (1 + np.exp(-10 * (score_all[~keep] - 0.7)))).sum() 
                # num_box_ratio = adj_box_num[conf_up].sum()/under
                # if under.item() > 0 :
                #     over_under_6.append(num_box_ratio.item())
                ...
                # print(conf)
                # exit()
                # print(torch.mean(reg))
                # print(torch.mean(reg2))
                # exit()
                ...
                # # pseudo_ious = 1 / (1 + np.exp(-100 * (pseudo_ious - 0.5)))
                # pseudo_ious2 = 1 / (1 + np.exp(-10 * (pseudo_ious2 - 0.5)))
                # com_iou = pseudo_ious
                # com_iou2 = pseudo_ious2
                # com_iou3 = pseudo_ious + 0.01*pseudo_ious2
                ...
                # com_iou5 = 0.5*pseudo_ious + 0.5*pseudo_ious2 ## 이걸로 변경경 aug3_3
                
            
                # # com_iou3 = pseudo_ious + 0.01*pseudo_ious2 + 0.1*torch.mean(reg)
                # # com_iou4 = pseudo_ious + 0.2*pseudo_ious2 + 0.1*torch.mean(reg)
                # # com_iou5 = pseudo_ious + 0.1*pseudo_ious2 + 0.1*torch.mean(reg)
                # # com_iou6 = pseudo_ious + 0.05*pseudo_ious2 + 0.1*torch.mean(reg)
                # # com_iou7 = pseudo_ious + 0.15*pseudo_ious2 + 0.1*torch.mean(reg)
                # # print(conf)
                # # print(conf-0.1*torch.mean(reg))
                # # print(reg)
                # # print(reg.shape)
                # # print(conf.shape)
                # # print(torch.mean(reg))
                # # exit()
                # # print(conf)
                # # print(conf/torch.mean(reg))
                # # exit()
                # # conf1 = conf-0.05*torch.mean(reg)
                # # print(conf1)
                # # conf1 = conf+0.1*reg2
                # # conf1  = 1 / (1 + np.exp(100 * (conf1 - 0.5)))
                # # conf2 = conf+0.05*reg2
                # # conf2  = 1 / (1 + np.exp(100 * (conf2 - 0.5)))
                # # conf3 = conf+0.15*reg2
                # # conf3  = 1 / (1 + np.exp(100 * (conf3 - 0.5)))
                # # conf1 = conf+0.1*abs(reg2)
                # # conf1  = 1 / (1 + np.exp(100 * (conf1 - 0.5)))
                # # conf2 = conf+0.05*abs(reg2)
                # # conf2  = 1 / (1 + np.exp(100 * (conf2 - 0.5)))
                # # conf3 = conf+0.15*abs(reg2)
                # # conf3  = 1 / (1 + np.exp(100 * (conf3 - 0.5)))
                
                # # print(reg2)
                # # print(conf)
                # # print(conf+0.5*reg2)
                # # print(conf+0.25*reg2)
                # #########원래 reg2 일 때
                # # reg2 = reg2.float()  
                # # non_zero_reg2 = reg2[reg2!=0]
                # # reg2[reg2==0] = non_zero_reg2.mean()
                # # reg2 = torch.tensor(reg2)
                # #########################
                # # exit()
                # # print(reg2)
                # # print(0.25*reg2)
                ...
                # # conf1 = conf-0.1*torch.mean(reg)
                # # conf1  = 1 / (1 + np.exp(100 * (conf1 - 0.5)))
                # # conf2 = conf-0.5*torch.mean(reg)
                # # conf2  = 1 / (1 + np.exp(100 * (conf2 - 0.5)))
                # # conf3 = conf-0.5*torch.mean(reg2)
                # # conf3  = 1 / (1 + np.exp(100 * (conf3 - 0.5)))
                # # conf4 = conf-0.1*torch.mean(reg2)
                # # conf4  = 1 / (1 + np.exp(100 * (conf4 - 0.5)))
                # conf1 = conf-0.6*torch.mean(reg)
                # conf1  = 1 / (1 + np.exp(100 * (conf1 - 0.5)))
                # conf2 = conf-0.5*torch.mean(reg)
                # conf2  = 1 / (1 + np.exp(100 * (conf2 - 0.5)))
                # conf3 = conf-0.6*torch.mean(reg2.float())
                # conf3  = 1 / (1 + np.exp(100 * (conf3 - 0.5)))
                # conf4 = conf-0.5*torch.mean(reg2.float())
                # conf4  = 1 / (1 + np.exp(100 * (conf4 - 0.5)))
                ...
                # count_soft = (conf2+com_iou)/2
                # count_soft2 = (conf2+com_iou2)/2
                # count_soft3 = (conf1+com_iou4)/2
                # count_soft4 = (conf1+com_iou5)/2
                # count_soft5 = (conf2+com_iou4)/2
                # count_soft6 = (conf2+com_iou5)/2
                # count_soft7 = (conf3+com_iou4)/2
                # count_soft8 = (conf3+com_iou5)/2
                # count_soft9 = (conf4+com_iou4)/2
                # count_soft10 = (conf4+com_iou5)/2
                # # print(count_soft5.sum()/keep.sum())
                # # exit()
                # # count_soft = conf * pseudo_ious
                # # if (count_soft.sum()/keep.sum()+0.1*torch.mean(reg)) > 0:
                # #     over_under.append(count_soft.sum()/keep.sum()+0.1*torch.mean(reg))

                # # if (count_soft2.sum()/keep.sum()+0.1*torch.mean(reg)) > 0:
                # #     over_under_2.append(count_soft2.sum()/keep.sum()+0.1*torch.mean(reg))

                # # if (count_soft3.sum()/keep.sum()+0.1*torch.mean(reg)) > 0:
                # #     over_under_3.append(count_soft3.sum()/keep.sum()+0.1*torch.mean(reg))

                # # if (count_soft4.sum()/keep.sum()+0.1*torch.mean(reg)) > 0:
                # #     over_under_4.append(count_soft4.sum()/keep.sum()+0.1*torch.mean(reg))

                # # if (count_soft5.sum()/keep.sum()+0.1*torch.mean(reg)) > 0:
                # #     over_under_5.append(count_soft5.sum()/keep.sum()+0.1*torch.mean(reg))

                # # if (count_soft6.sum()/keep.sum()+0.1*torch.mean(reg)) > 0:
                # #     over_under_6.append(count_soft6.sum()/keep.sum()+0.1*torch.mean(reg))
                # # if (count_soft7.sum()/keep.sum()+0.1*torch.mean(reg)) > 0:
                # #     over_under_7.append(count_soft7.sum()/keep.sum()+0.1*torch.mean(reg))
                # # print(keep.sum())
                # # keep = conf.clone()
                # # print(keep.sum())
                # # exit()
                ...
                # if count_soft8.sum()/keep.sum() > 0:
                #     over_under_8.append(count_soft8.sum()/keep.sum())
                # if count_soft9.sum()/keep.sum() > 0:
                #     over_under_9.append(count_soft9.sum()/keep.sum())
                # if count_soft10.sum()/keep.sum() > 0:
                #     over_under_10.append(count_soft10.sum()/keep.sum())
                ###################################################################
                ...
                # conf1 = conf-0.7*torch.mean(reg)
                # conf1  = 1 / (1 + np.exp(100 * (conf1 - 0.5)))
                # conf1 = conf-0.7*reg2
                # conf1 = conf-0.5*reg2
                conf1 = conf
                # conf1  = 1 / (1 + np.exp(100 * (conf1 - 0.5)))
                conf1  = 1 / (1 + np.exp(100 * (conf1 - 0.7)))
                
                # #iou
                # com_iou1 = pseudo_ious
                # com_iou1 = 1 / (1 + np.exp(-100 * (com_iou1 - 0.5)))
                # com_iou2 = pseudo_ious
                # com_iou2 = 1 / (1 + np.exp(-100 * (com_iou2 - 0.6)))    
                # com_iou3 = pseudo_ious
                # com_iou3 = 1 / (1 + np.exp(-100 * (com_iou3 - 0.4)))   
                # com_iou4 = pseudo_ious
                # com_iou4 = 1 / (1 + np.exp(-70 * (com_iou4 - 0.5)))
                # com_iou5 = pseudo_ious
                # com_iou5 = 1 / (1 + np.exp(-50 * (com_iou5 - 0.5)))
                # com_iou6 = pseudo_ious
                # com_iou6 = 1 / (1 + np.exp(-20 * (com_iou6 - 0.5)))

                # count_soft = (conf1+com_iou1)/2
                # count_soft2 = (conf1+com_iou2)/2
                # count_soft3 = (conf1+com_iou3)/2
                # count_soft4 = (conf1+com_iou4)/2
                # count_soft5 = (conf1+com_iou5)/2
                # count_soft6 = (conf1+com_iou6)/2
                # if count_soft.sum()/keep.sum() > 0:
                #     over_under.append(count_soft.sum()/keep.sum())
                ...
                #dist
                # com_iou1 = pseudo_ious2
                # com_iou1 = 1 / (1 + np.exp(-100 * (com_iou1 - 0.5)))
                # com_iou2 = pseudo_ious2
                # com_iou2 = 1 / (1 + np.exp(-100 * (com_iou2 - 0.6)))    
                # com_iou3 = pseudo_ious2
                # com_iou3 = 1 / (1 + np.exp(-100 * (com_iou3 - 0.4)))   
                # com_iou4 = pseudo_ious2
                # com_iou4 = 1 / (1 + np.exp(-70 * (com_iou4 - 0.5)))
                # com_iou5 = pseudo_ious2
                # com_iou5 = 1 / (1 + np.exp(-50 * (com_iou5 - 0.5)))
                # com_iou6 = pseudo_ious2
                # com_iou6 = 1 / (1 + np.exp(-20 * (com_iou6 - 0.5)))

                #dist
                con2 = pseudo_ious2
                # con2 = 1 / (1 + np.exp(-100 * (con2 - 0.6))) 

                con1 = pseudo_ious
                

                com_iou1 = 0.5*con1 + 0.5*con2
                # com_iou1 = con1 * con2
                com_iou2 = con1 + 0.2*con2
                com_iou3 = 0.2*con1 + con2
                com_iou4 = 0.8*con1 + 0.2*con2
                com_iou5 = 0.2*con1 + 0.8*con2
                ...
                conf2 = conf
                # conf1  = 1 / (1 + np.exp(100 * (conf1 - 0.5)))
                conf2  = 1 / (1 + np.exp(100 * (conf2 - 0.7)))
                ...
                # count_soft = (conf2+com_iou1)/2
                # count_soft = com_iou1 * conf2
                count_soft = com_iou1
                count_soft2 = (conf2+com_iou2)/2
                count_soft3 = (conf2+com_iou3)/2
                count_soft4 = (conf2+com_iou4)/2
                count_soft5 = (conf2+com_iou5)/2
                
                # if count_soft.sum()/keep.sum() > 0:
                #     over_under_7.append(count_soft.sum()/keep.sum())

                if count_soft.sum() > 0:
                    over_under_7.append(count_soft.sum())

                if count_soft2.sum()/keep.sum() > 0:
                    over_under_8.append(count_soft2.sum()/keep.sum())

                if count_soft3.sum()/keep.sum() > 0:
                    over_under_9.append(count_soft3.sum()/keep.sum())

                if count_soft4.sum()/keep.sum() > 0:
                    over_under_10.append(count_soft4.sum()/keep.sum())

                if count_soft5.sum()/keep.sum() > 0:
                    over_under_11.append(count_soft5.sum()/keep.sum())
...
# 02.21/r50_retina_consistency_no_conf/
            # NAME_DIR  = "./02.13/r50_retina_giou_ori/" + str(args.tag1)
            # NAME_DIR  = "./02.17/r50_retina/" + str(args.tag1)  #이거는 황중원님이 reliability 작성하신 대로 돌린거거
            NAME_DIR  = "./02.21/r50_retina_ori_dataset/" + str(args.tag1)
