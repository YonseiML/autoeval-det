...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            ## 추가 ##    
            _, index = torch.sort(remove_score, descending=True)
            slide = removed_box.shape[0]
            if iou_mean.item() < 0.6:
                
                ######반씩 없애기########
                # while True:
                #     slide = slide/2
                #     select_index = index[:int(slide)]
                #     selected_box = removed_box[select_index]
                #     x1_min = torch.min(selected_box[:, 0])
                #     y1_min = torch.min(selected_box[:, 1])
                    
                #     # 각 bbox의 우하단 좌표 (x2, y2)
                #     x2_max = torch.max(selected_box[:, 2])
                #     y2_max = torch.max(selected_box[:, 3])
                    
                #     # 하나의 외접 bbox 반환
                #     pseudo_bbox = torch.tensor([x1_min, y1_min, x2_max, y2_max])
                #     removed_iou = torchvision.ops.box_iou(selected_box, selected_box)
                #     iou_mean = torch.mean(removed_iou)
                #     if iou_mean.item() >= 0.7:
                #         break
                #########################
                ######ori#####
...
def apply_iou_filter2(scores: torch.Tensor,
              iou_values_all: torch.Tensor,
              iou_threshold: float,
              init_box: torch.Tensor) -> torch.Tensor:
    """
    ...
    for i in range(num_boxes):
        ...
        # 각 bbox의 좌상단 좌표 (x1, y1)
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            iou_ref_remove = iou_values_all[current_idx][iou_with_remaining]
            iou_ave = torch.mean(iou_values_all[current_idx][iou_with_remaining])
            pseudo_ious.append(iou_ave.item())

            if removed_box.shape[0] == 1:
                pseudo_ious2.append(0)
            else:
                removed_iou = torchvision.ops.box_iou(removed_box, removed_box)  # 이 경우 removed_box가 하나인 경우면 1임
                iou_mean = torch.mean(removed_iou)
                pseudo_ious2.append(iou_mean.item())
            ...
            no += 1
            pseudo_ious.append(0)  # 이거 나중에 1으로도 해보기
            pseudo_ious2.append(0)
            # pseudo_ious.append(1)
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                if (score_up_down.sum()+score_down_up.sum())/keep.sum() > 0:
                    over_under.append((score_up_down.sum()+score_down_up.sum())/keep.sum())
                if (score_down_up.sum()/keep.sum()) > 0 :
                    over_under_2.append(score_down_up.sum()/keep.sum())
                
                if (score_up_down2.sum()+score_down_up2.sum())/keep.sum() > 0:
                    over_under_3.append((score_up_down2.sum()+score_down_up2.sum())/keep.sum())
                ...
                if ((score_up.sum()+score_down_up.sum())/keep.sum()) > 0 :
                    over_under_5.append((score_up.sum()+score_down_up.sum())/keep.sum())
                if ((score_up2.sum()+score_down_up2.sum())/keep.sum()) > 0 :
                    over_under_6.append((score_up2.sum()+score_down_up2.sum())/keep.sum())
                ###########res_consistency_2##############
                '''
                keep, pseudo_ious = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                keep2, pseudo_ious2, pseudo_ious2_2 = apply_iou_filter2(score_all, iou_all, 0.5, init_box)
                conf = torch.tensor(results_cls_per_img[img_idx])
                ...
                conf_up2 = conf > 0.5
                conf_down2 = conf <= 0.5
                iou_up2 = pseudo_ious2 > 0.7
                iou_down2 = pseudo_ious2 <= 0.7
                iou_up2_2 = pseudo_ious2_2 > 0.7
                iou_down2_2 = pseudo_ious2_2 <= 0.7

                score_up2 = conf_up2 & iou_up2
                score_up_down2 = conf_up2 & iou_down2
                score_down_up2 = conf_down2 & iou_up2
                score_down2 = conf_down2 & iou_down2

                score_up2_2 = conf_up2 & iou_up2_2
                score_up_down2_2 = conf_up2 & iou_down2_2
                score_down_up2_2 = conf_down2 & iou_up2_2
                score_down2_2 = conf_down2 & iou_down2_2
                ...
                if (score_up_down2_2.sum()+score_down_up2_2.sum())/keep.sum() > 0:
                    over_under_5.append((score_up_down2_2.sum()+score_down_up2_2.sum())/keep.sum())
                if (score_down_up2_2.sum()/keep.sum()) > 0 :
                    over_under_6.append(score_down_up2_2.sum()/keep.sum())
                
                if ((score_up.sum()+score_down_up.sum())/keep.sum()) > 0 :
                    over_under_7.append((score_up.sum()+score_down_up.sum())/keep.sum())
                if ((score_up2.sum()+score_down_up2.sum())/keep.sum()) > 0 :
                    over_under_8.append((score_up2.sum()+score_down_up2.sum())/keep.sum())
                if ((score_up2_2.sum()+score_down_up2_2.sum())/keep.sum()) > 0 :
                    over_under_9.append((score_up2_2.sum()+score_down_up2_2.sum())/keep.sum())
                '''            
            #####################################################
        ...
        least_iou_cost_final[area_idx] = least_iou_cost_final[area_idx] / (num_flag[area_idx]) # BS
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_test/" + str(args.tag1)
