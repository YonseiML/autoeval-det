...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            #     # print(pseudo_iou)
            # wbf version
                predicted_bbox = torch.tensor([x1/img_w, y1/img_h, x2/img_w, y2/img_h]).cuda()
            pseudo_iou = torchvision.ops.box_iou(predicted_bbox.unsqueeze(0) , pseudo_bbox.unsqueeze(0))
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep, pseudo_ious, pseudo_ious2 = apply_iou_filter(score_all, iou_all, 0.5, init_box, img_size)
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_revise_resize_select_wbf/" + str(args.tag1)
