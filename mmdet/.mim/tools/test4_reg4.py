...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                conf1 = conf-0.01*torch.mean(reg)
                conf1  = 1 / (1 + np.exp(100 * (conf1 - 0.5)))
                conf2 = conf-0.5*torch.mean(reg)
                conf2  = 1 / (1 + np.exp(100 * (conf2 - 0.5)))
                conf3 = conf-0.5*reg2
                conf3  = 1 / (1 + np.exp(100 * (conf3 - 0.5)))
                conf4 = conf-0.01*reg2
                conf4  = 1 / (1 + np.exp(100 * (conf4 - 0.5)))
                conf5 = conf-0.05*reg2
                conf5  = 1 / (1 + np.exp(100 * (conf5 - 0.5)))
                ...
                count_soft2 = (conf1+com_iou5)/2
                count_soft3 = (conf2+com_iou5)/2
                count_soft4 = (conf3+com_iou5)/2
                count_soft5 = (conf4+com_iou5)/2
                count_soft6 = (conf5+com_iou5)/2
                count_soft7 = (conf1+com_iou7)/2
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./01.21/01.19/res_ours1_revise_no_resize_add_reg4/" + str(args.tag1)
