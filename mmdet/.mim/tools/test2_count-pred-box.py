...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        # removed_box = init_box[iou_with_remaining]  # 앞서 삭제된 bbox도 포함
        # removed_box = init_box[remove_idx.bool()]  # res_1017_new랑 res_1017_new_2 일 때는 이렇게
        removed_box = init_box[iou_with_remaining] # res_1017_new_all
        # removed_score = scores[remove_idx.bool()]
        removed_score = scores[iou_with_remaining]
        
    
        # ref_iou = iou_values_all[current_idx][iou_with_remaining] # 앞서 삭제된 bbox도 포함
        # ref_iou = iou_values_all[current_idx][remove_idx.bool()]    ############version1일 때 res_1017_new  
        # ref_iou = ref_iou *removed_score  ############version1일 때 res_1017_new_2
        ##################################################################
        ...
        # ref_box_area = torchvision.ops.box_area(pseudo_bbox.unsqueeze(0))
            
            # 교집합 면적 / box1의 면적
        ref_iou = intersection / ref_box_area[:, None]
        # ref_iou = iou_values_all[current_idx][iou_with_remaining] # res_1017_new_all
        ref = scores[current_idx]
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            if ref > 0.5 :
                # print(ref_iou.shape[0])
                iou_up7 = ref_iou > 0.7
                iou_down7 = ref_iou <= 0.7
                if iou_up7.sum().item() > int(ref_iou.shape[0]/2) :
                    score_up7 += 1
                else:
                    score_up_down7 += 1
                # score_up7 += iou_up7.sum().item()
                # score_up_down7 += iou_down7.sum().item()

                iou_up8 = ref_iou > 0.8
                iou_down8 = ref_iou <= 0.8
                if iou_up8.sum().item() > int(ref_iou.shape[0]/2) :
                    score_up8 += 1
                else:
                    score_up_down8 += 1
                # score_up8 += iou_up8.sum().item()
                # score_up_down8 += iou_down8.sum().item()

                iou_up9 = ref_iou > 0.9
                iou_down9 = ref_iou <= 0.9
                if iou_up9.sum().item() > int(ref_iou.shape[0]/2) :
                    score_up9 += 1
                else:
                    score_up_down9 += 1
                # score_up9 += iou_up9.sum().item()
                # score_up_down9 += iou_down9.sum().item()
            else : 
                iou_up7 = ref_iou > 0.7
                iou_down7 = ref_iou <= 0.7
                if iou_up7.sum().item() > int(ref_iou.shape[0]/2) :
                    ...
                    score_down7 += 1
                # score_down_up7 += iou_up7.sum().item()
                # score_down7 += iou_down7.sum().item()
                ...
                if iou_up8.sum().item() > int(ref_iou.shape[0]/2) :
                    ...
                    score_down8 += 1
                # score_down_up8 += iou_up8.sum().item()
                # score_down8 += iou_down8.sum().item()
                ...
                if iou_up9.sum().item() > int(ref_iou.shape[0]/2) :
                    ...
                    score_down9 += 1
                # score_down_up9 += iou_up9.sum().item()
                # score_down9 += iou_down9.sum().item()
        else:
            # nothing += 1
            if ref > 0.5 :
                score_up_down7 += 1
    ...
    score7 = score_up7, score_up_down7, score_down_up7, score_down7
    score8 = score_up8, score_up_down8, score_down_up8, score_down8
    score9 = score_up9, score_up_down9, score_down_up9, score_down9
    return keep, score7, score8, score9
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep, score7, score8, score9 = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                score_up7, score_up_down7, score_down_up7, score_down7 = score7
                score_up8, score_up_down8, score_down_up8, score_down8 = score8 
                score_up9, score_up_down9, score_down_up9, score_down9 = score9
                # print(score_up7)
                # print(score_up_down7)
                # print(score_down_up7)
                # print(score_down7)
                # exit()
                # denominator = (score_all.shape[0] - keep.sum())  # res_1017_new랑 res_1017_new_2랑 res_1017_new_all 일 때는 이렇게
                # denominator = score_up7+score_up_down7+score_down_up7+score_down7 # res_1017_new_all_modi
                # print(denominator)
                # print(score_up8+score_up_down8+score_down_up8+score_down8)
                # print(score_up9+score_up_down9+score_down_up9+score_down9)
                # exit()
                if score_up7/keep.sum() > 0:
                    over_under.append(score_up7/keep.sum())
                if score_up_down7/keep.sum() > 0:
                    over_under_2.append(score_up_down7/keep.sum())
                if score_down_up7/keep.sum() > 0:
                    over_under_3.append(score_down_up7/keep.sum())
                if score_down7/keep.sum() > 0:
                    over_under_4.append(score_down7/keep.sum())

                if score_up8/keep.sum() > 0:
                    over_under_5.append(score_up8/keep.sum())
                if score_up_down8/keep.sum() > 0:
                    over_under_6.append(score_up_down8/keep.sum())
                if score_down_up8/keep.sum() > 0:
                    over_under_7.append(score_down_up8/keep.sum())
                if score_down8/keep.sum() > 0:
                    over_under_8.append(score_down8/keep.sum())
                
                if score_up9/keep.sum() > 0:
                    over_under_9.append(score_up9/keep.sum())
                if score_up_down9/keep.sum() > 0:
                    over_under_10.append(score_up_down9/keep.sum())
                if score_down_up9/keep.sum() > 0:
                    over_under_11.append(score_down_up9/keep.sum())
                if score_down9/keep.sum() > 0:
                    over_under_12.append(score_down9/keep.sum())


                ################################
                # if (score_up_down+score_down_up)/denominator > 0:
                #     over_under_5.append((score_up_down+score_down_up)/denominator)
                # if (score_up+score_down)/denominator > 0:
                #     over_under_6.append((score_up+score_down)/denominator)
                # if 1-(score_down_up/denominator) > 0:
                #     over_under_7.append(1-(score_down_up/denominator))
