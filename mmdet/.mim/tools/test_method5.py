...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                # print(iou_out_ex[0])
                iou_ex = torch.tensor(np.array(iou_ex))
                jaccard_dist = torch.ones(len(iou_ex), len(iou_ex)) - iou_ex
                # print(iou_ex[0])
                # print(jaccard_dist[0])

                mask = torch.tril(torch.ones(len(jaccard_dist), len(jaccard_dist)), diagonal=-1)
                masked_jaccard_dist = jaccard_dist * mask + (1 - mask) * 999
                # print(masked_jaccard_dist[0])
                # print(masked_jaccard_dist[4])
                j_min, min_idx = torch.min(masked_jaccard_dist, dim=-1)
                # print(j_min.shape)
                # print(j_min)
                # exit()
                # 모델을 fit
                beta_cal.fit(score_ex, j_min)

                # 보정된 확률 예측
                calibrated_scores = beta_cal.predict(score_ex)
                # print("보정된 confidence score: ", calibrated_scores)
                # print(score_ex)
                # # exit()
                # print(torch.std(score_ex))
                # print(torch.std(calibrated_scores))
                diff = (score_ex.cuda() - calibrated_scores) ** 2
                # exit()
                if torch.std(score_ex)> 0 :
                    over_under.append(torch.std(score_ex)) # 원래 score의 std
                if torch.std(calibrated_scores)> 0 :
                    over_under_2.append(torch.std(calibrated_scores).cpu()) # 수정된 score의 std
                if torch.mean(calibrated_scores)> 0 :
                    over_under_3.append(torch.mean(calibrated_scores).cpu())
                if torch.std(score_ex) - torch.std(calibrated_scores) > 0:
                    over_under_3_2.append((torch.std(score_ex) - torch.std(calibrated_scores).cpu())**2)
                if torch.std(diff)> 0 :
                    over_under_4.append(torch.std(diff).cpu())
                if torch.mean(diff)> 0 :
                    over_under_4_2.append(torch.mean(diff).cpu())
            # exit()
            #####################################################
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_0930_2_modi/" + str(args.tag1)
