...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                if (score_down_up.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2)) > 0 :
                    over_under_2.append(score_down_up.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2))
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./01.21/01.18/res_bos_Replication_aug2/" + str(args.tag1)
