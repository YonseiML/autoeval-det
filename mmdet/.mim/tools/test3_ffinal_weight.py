...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            if iou_mean.item() < 0.7:   # 0.65이 베스트 0.7
            # if (iou_mean.item() < 0.7) & (inter_over_pseudo_box.item() >= 2/3):
                ...
                # 새로운 bbox
                pseudo_bbox7 = torch.tensor([new_x1, new_y1, new_x2, new_y2])
                pseudo_iou7 = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox7.unsqueeze(0))
                pseudo_ious.append(pseudo_iou7.item())
            else:
                pseudo_iou7 = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox.unsqueeze(0))
                pseudo_ious.append(pseudo_iou7.item())
    ...
    return keep, torch.tensor(pseudo_ious), torch.tensor(pseudo_ious2), torch.tensor(adj_iou)
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                # print(score_all.shape)
                # exit()
                # cv_conf_b = torch.std(score_all)/torch.mean(score_all)
                # cv_iou_b = torch.std(iou_all)/torch.mean(iou_all)
                ...
                keep, pseudo_ious, pseudo_ious2, adj_iou = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                # exit()
                # print(conf)
                pseudo_ious2 = 0.9 * pseudo_ious2 + 0.1 * adj_iou
                pseudo_ious = 0.8 * pseudo_ious2 + 0.2 * adj_iou
                ...
                # exit()
                # cv_conf = torch.std(score_all)/torch.mean(score_all)
                # cv_iou = torch.std(iou_all)/torch.mean(iou_all)
                # over_under_13.append(cv_conf_b/cv_iou_b)
                # over_under_14.append(cv_conf/cv_iou)
                conf_up = conf > 0.5
                conf_down = conf <= 0.5
                ...
                # adj_up = adj_iou > 0.7
                # score_down_up  = score_down_up & adj_up
                # score_down_up2 = score_down_up2 & adj_up
                ...
                # print(score_down_up2.sum()) #51 -> 73   # 20
                # print(score_down2.sum()) #19 -> 31
                # print(keep.sum()) #84 -> 107
                # print(score_down_up2.sum()/keep.sum())  #0.3056  0.4138 0.1495  : 0.6
                # # 0.0278  0.1034 0.0 : 0.8
                # # 0.2500 0.2759  0.5327 : adj_iou down 
                # # 0.5556  0.6897 0.6822 : original
                # exit()
                ...
                if (score_down.sum()/keep.sum()) > 0 :
                    over_under_7.append(score_down.sum()/keep.sum())
                ...
                if (score_up2.sum()/keep.sum()) > 0 :
                    over_under_10.append(score_up2.sum()/keep.sum())

                if (score_up_down.sum()/keep.sum()) > 0 :
                    over_under_11.append(score_up_down.sum()/keep.sum())
                if (score_up_down2.sum()/keep.sum()) > 0 :
                    over_under_12.append(score_up_down2.sum()/keep.sum())
            #####################################################
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_ffinal_weight/" + str(args.tag1)
