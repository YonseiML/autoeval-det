...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            boxes_list = [np.array(removed_box).tolist()]
            scores_list = [np.array(remove_score).tolist()]
            labels_list = [np.array(torch.zeros(removed_box.shape[0])).tolist()]
            
            pseudo_bbox, final_scores, final_labels = weighted_boxes_fusion_raw(boxes_list, scores_list, labels_list, iou_thr=0, skip_box_thr=0.0001)
            # print(pseudo_bbox)
            # print(final_scores)
            # exit()
            # exit()
            ...
            pseudo_bbox = torch.tensor(pseudo_bbox[0])
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                # com_iou3 = 0.8*pseudo_ious + 0.2*pseudo_ious2
                com_iou3 = 0.9*pseudo_ious + 0.1*pseudo_ious2
                # com_iou3 = pseudo_ious * pseudo_ious2
                com_iou4 = pseudo_ious + 0.2*pseudo_ious2
                com_iou5 = pseudo_ious + 0.1*pseudo_ious2
                com_iou6 = pseudo_ious + 0.3*pseudo_ious2
                com_iou7 = pseudo_ious + 0.4*pseudo_ious2
                com_iou8 = pseudo_ious + 0.01*pseudo_ious2
                com_iou9 = pseudo_ious + 0.1*pseudo_ious2
                com_iou10 = pseudo_ious + 0.05*pseudo_ious2
                ...
                com_iou8 = 1 / (1 + np.exp(-100 * (com_iou8 - 0.5)))
                com_iou9 = 1 / (1 + np.exp(-100 * (com_iou9 - 0.5)))
                com_iou10 = 1 / (1 + np.exp(-100 * (com_iou10 - 0.5)))
                ...
                count_soft8 = (conf+com_iou8)/2
                count_soft9 = (conf+com_iou9)/2
                count_soft10 = (conf+com_iou10)/2
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_ours1_revise_no_resize_final_wbf_max300/" + str(args.tag1)
