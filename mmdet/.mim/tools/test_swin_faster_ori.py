...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            model_score = abs(abs(predicted_score-remove_score)-(1-pre_iou.squeeze(0)))
            # reg.append(((model_score>0.2).sum()/remove_score.shape[0]).item())
            reg.append(torch.mean(model_score).item())
    ...
    return keep, torch.tensor(pseudo_ious), torch.tensor(pseudo_ious2), torch.tensor(reg)
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            # print(score_all.shape)  # torch.Size([1000])
            # print(score_ex.shape)  # torch.Size([20])
            # a = torch.tensor(results_bbox_per_img[img_idx])
            # print(a.shape)  # torch.Size([20, 4])
            # print(init_box.shape) # torch.Size([262, 4])
            # print(len(iou_all))  #262
            # print(len(iou_ex))  #20
            # exit() 
            if len(iou_all) > 0 :
                ...
                keep, pseudo_ious, pseudo_ious2, reg = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                count_soft8 = (conf2+com_iou5)/2
                count_soft9 = (conf2+com_iou7)/2
                ...
                if count_soft8.sum()/keep.sum() > 0:
                    over_under_11.append(count_soft8.sum()/keep.sum())
                if count_soft9.sum()/keep.sum() > 0:
                    over_under_12.append(count_soft9.sum()/keep.sum())
                ################################final은 여기까지만 
                com_iou5 = pseudo_ious + 0.1*pseudo_ious2
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./02.04/01.31/car_swin_faster_rcnn/" + str(args.tag1)
