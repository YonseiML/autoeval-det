...
def main():
    ...
    result_over_under_3_2 = [0 for area_idx in areaRng]
    result_over_under_4_2 = [0 for area_idx in areaRng]
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            # print(len(results_cls_per_img[img_idx]))  # 107
            ...
            if len(iou_all) > 0 :
                ...
                keep, under_over_num = apply_iou_filter(score_all, iou_all, 0.5)
                iou_ex = iou_all[keep.bool(), :][:, keep.bool()]
                # iou_ex = iou_exs[img_idx]
                iou_ex_2 = iou_all[keep.bool(), :]   # 이거 쓰는 거 안 좋게 나옴
                iou_re = iou_all[~keep.bool(),:]
                # iou_ex = iou_exs[img_idx]
                iou_mean = torch.mean(iou_all, dim=-1)
                iou_ex_mean = torch.mean(iou_ex, dim=-1)
                iou_ex_2_mean = torch.mean(iou_ex_2, dim=-1)
                iou_re_mean = torch.mean(iou_re, dim=-1)
            # iou_mean = torch.mean(torch.tensor(np.array(iou_all)), dim=-1)
            # iou_ex_mean = torch.mean(torch.tensor(np.array(iou_ex)), dim=-1)
            # exit()
            # print(torch.mean(iou_mean))  # tensor(0.0604)
            # print(torch.mean(iou_ex_mean))  # tensor(0.0224)
            # print(torch.mean(iou_re_mean))   # tensor(0.0660)
            
            #######################################
            # score_all = score_all[score_all>0.05]
            # keep_first, under_over_num = apply_iou_filter(score_all, iou_all, 0.5)
            # keep_second, under_over_num = apply_iou_filter_no_sort(score_all, iou_all, 0.5)
            # score_first = score_all[keep_first.bool()]
            # score_seconde = score_all[keep_second.bool()]
            # var_exist = torch.var(score_first.sort(descending=True).values-score_seconde.sort(descending=True).values)
            
            # if var_exist > 0 :
            #     over_under.append(var_exist)
            ##########################
                ## res0909_modi
                # if torch.std(score_all/iou_mean) > 0:
                #     over_under.append(torch.std(score_all/iou_mean))
                #     # print(torch.std(score_all/iou_mean))
                # if torch.mean(score_all/iou_mean) >0 :
                #     over_under_2.append(torch.mean(score_all/iou_mean))
                # if torch.std(score_ex/iou_ex_mean) >0 :
                #     over_under_3.append(torch.std(score_ex/iou_ex_mean))
                # if torch.mean(score_ex/iou_ex_mean) >0 :
                #     over_under_4.append(torch.mean(score_ex/iou_ex_mean))
                # if torch.std(score_ex/iou_ex_2_mean) >0 :
                #     over_under_3_2.append(torch.std(score_ex/iou_ex_2_mean))
                # if torch.mean(score_ex/iou_ex_2_mean) >0 :
                #     over_under_4_2.append(torch.mean(score_ex/iou_ex_2_mean))

                ## res0930
                # if torch.std(score_all/iou_mean) > 0:
                #     over_under.append(torch.std(score_all))
                # # print(torch.std(score_all/iou_mean))
                # if torch.mean(score_all/iou_mean) >0 :
                #     over_under_2.append(torch.mean(score_all))
                # if torch.std(score_ex/iou_ex_mean) >0 :
                #     over_under_3.append(torch.std(score_ex))
                # if torch.mean(score_ex/iou_ex_mean) >0 :
                #     over_under_4.append(torch.mean(score_ex))
                # if torch.std(score_ex/iou_ex_2_mean) >0 :
                #     over_under_3_2.append(torch.std(score_ex))
                # if torch.mean(score_ex/iou_ex_2_mean) >0 :
                #     over_under_4_2.append(torch.mean(score_ex))

            
            ##########################
            # score_all = score_all[score_all>0.05]
            # keep, under_over_num = apply_iou_filter(score_all, iou_all, 0.5)
            # score_rem = score_all[~keep.bool()]
            # if torch.mean(score_ex)/torch.mean(score_rem) > 0:
            #     over_under.append(torch.mean(score_ex)/torch.mean(score_rem))   # R^2 : 0.673
            ##########################            
            ################ 보통 아래 두 개 같이################
            # score_all = score_all[score_all>0.05]
            # keep_idx, under_over_num = apply_iou_filter(score_all, iou_all, 0.5)
           
            ######################## method 4
            # remove_num = len(score_all) - len(score_ex)
            # if remove_num == 0:
            #     over_under.append(0)
            # else:
            #     over_under.append(under_over_num)
            #################### method1
            # remove_num = len(score_all) - len(score_ex)
            # if remove_num == 0:
            #     over_under.append(0)
            # else:
            #     over_under.append(under_over_num/remove_num)
        ...
        result_over_under_3_2[area_idx] = np.mean(over_under_3_2).item()
        result_over_under_4_2[area_idx] = np.mean(over_under_4_2).item()
    print()
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            # NAME_DIR  = "./res_0909_modi/" + str(args.tag1)
            NAME_DIR  = "./res_0930/" + str(args.tag1)
            ...
            with open(NAME, "r", encoding="utf-8") as f:
                ...
                if data == {}:
                    data.update({0: [[metric['bbox_mAP'], metric['bbox_mAP_50'], metric['bbox_mAP_75'], metric['bbox_mAP_s'], metric['bbox_mAP_m'], metric['bbox_mAP_l']], least_cost, \
                        least_iou_cost_final,  result_over_under, result_over_under_2, result_over_under_3,result_over_under_4, result_over_under_3_2,result_over_under_4_2, least_reg_cost_final, least_cls_cost_final, num_cls_large_k, mean_cls_large_k, results_cls_entropy_small_k]})
                else:
