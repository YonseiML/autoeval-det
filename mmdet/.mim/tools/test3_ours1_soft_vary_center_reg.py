...
def main():
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                if count_soft.sum()/keep.sum() > 0:
                    over_under.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2))


                count_soft = (conf1**0.6) * (pseudo_ious1**0.4)
                if count_soft.sum()/keep.sum() > 0:
                    over_under_2.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2))

                count_soft = (conf1**0.4) * (pseudo_ious1**0.6)
                if count_soft.sum()/keep.sum() > 0:
                    over_under_3.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2))
                ...
                if count_soft.sum()/keep.sum() > 0:
                    over_under_4.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2))


                count_soft = (conf2**0.6) * (pseudo_ious2**0.4)
                if count_soft.sum()/keep.sum() > 0:
                    over_under_5.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2))

                count_soft = (conf2**0.4) * (pseudo_ious2**0.6)
                if count_soft.sum()/keep.sum() > 0:
                    over_under_6.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2))
                ...
                if count_soft.sum()/keep.sum() > 0:
                    over_under_7.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2))


                count_soft = (conf3**0.6) * (pseudo_ious3**0.4)
                if count_soft.sum()/keep.sum() > 0:
                    over_under_8.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2))

                count_soft = (conf3**0.4) * (pseudo_ious3**0.6)
                if count_soft.sum()/keep.sum() > 0:
                    over_under_9.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2))
                ...
                count_soft = conf * pseudo_ious
                if count_soft.sum()/keep.sum() > 0:
                    over_under_10.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2))


                count_soft = (conf**0.6) * (pseudo_ious**0.4)
                if count_soft.sum()/keep.sum() > 0:
                    over_under_11.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2))

                count_soft = (conf**0.4) * (pseudo_ious**0.6)
                if count_soft.sum()/keep.sum() > 0:
                    over_under_12.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2))
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_ours1_soft_vary_center_reg/" + str(args.tag1)
