...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                conf1  = 1 / (1 + np.exp(100 * (conf - 0.6)))   #0.6
                pseudo_ious1 = 1 / (1 + np.exp(-100 * (pseudo_ious - 0.5)))  #0.6

                count_soft = conf1 * pseudo_ious1
                if count_soft.sum()/keep.sum() > 0:
                    over_under.append(count_soft.sum()/keep.sum())

                conf1  = 1 / (1 + np.exp(150 * (conf - 0.6)))
                pseudo_ious1 = 1 / (1 + np.exp(-150 * (pseudo_ious - 0.5)))

                count_soft = conf1 * pseudo_ious1
                if count_soft.sum()/keep.sum() > 0:
                    over_under_2.append(count_soft.sum()/keep.sum())

                conf1  = 1 / (1 + np.exp(200 * (conf - 0.6)))
                ...
                conf1  = 1 / (1 + np.exp(100 * (conf - 0.5)))        #0.4
                pseudo_ious1 = 1 / (1 + np.exp(-100 * (pseudo_ious - 0.6)))  #0.4

                count_soft = conf1 * pseudo_ious1
                if count_soft.sum()/keep.sum() > 0:
                    over_under_4.append(count_soft.sum()/keep.sum())

                conf1  = 1 / (1 + np.exp(150 * (conf - 0.5)))
                pseudo_ious1 = 1 / (1 + np.exp(-150 * (pseudo_ious - 0.6)))
                ...
                pseudo_ious1 = 1 / (1 + np.exp(-200 * (pseudo_ious - 0.6)))
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./01.14/01.13/res_ours1_soft2/" + str(args.tag1)
