...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        agnostic_nms = (0.5 + 0.5 / (1 + np.exp(-5 * (remove_score - 0.6)))).sum()
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                #soft version
                under = (0.5 + 0.5 / (1 + np.exp(-5 * (score_all - 0.6)))).sum()
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./02.04/01.31/car_swin_faster_rcnn3/" + str(args.tag1)
