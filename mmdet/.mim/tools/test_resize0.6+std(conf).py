...
def main():
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                if (1-(score_down_up.sum()/keep.sum())) > 0 :
                    over_under_5.append(1-(score_down_up.sum()/keep.sum()))
                if torch.std(score_all).item() > 0:
                    over_under_6.append(torch.std(score_all).item())
            #####################################################
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_resize0.6/" + str(args.tag1)
