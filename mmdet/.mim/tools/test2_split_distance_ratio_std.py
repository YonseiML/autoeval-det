...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            distance2 = torch.sqrt((center1_x - center2_x)**2 + (center1_y - center2_y)**2)
            # print(distance2)
            std_dist2 = torch.std(distance2)
            # print(std_dist2)
            if removed_box.shape[0] == 1:
                std_dist.append(0)
            else:
                std_dist.append(std_dist2.item())
            # exit()
            x1_min, y1_min, x2_max, y2_max = ref_bbox
            ref_width = x2_max-x1_min
            ref_height = y2_max-y1_min
            ref_length = max(ref_width, ref_height)
            ref_length = ref_length / 2
            distance = distance / ref_length
            pseudo_ious.append(distance.item())
            
            # width_ratio = pseudo_width / ref_width
            # height_ratio = pseudo_height / ref_height
            # print(width_ratio)
            # print(height_ratio)
            # exit()
            # pseudo_ious.append(inter_over_pseudo_box.item())
    ...
    # print(pseudo_ious)
    # pseudo_ious = torch.tensor(pseudo_ious)
    # a = pseudo_ious <= 0.5
    # print(a.sum())
    # b = pseudo_ious <= 0.25
    # print(b.sum())
    ...
    return keep, torch.tensor(pseudo_ious), torch.tensor(std_dist)
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep, pseudo_ious, std_dist = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                h, w, _ =  dataset.prepare_test_img(img_idx)['img_metas'][0].data['img_shape']
                # print(pseudo_ious)
                # print(h, w)
                # print((pseudo_ious<5).sum())
                # exit()
                conf_up = conf > 0.5
                conf_down = conf <= 0.5
                # conf_up = conf > 0.6
                ...
                iou_down = pseudo_ious <= 0.5    #나오는 순서대로 -0.728/-0.825  -> conf 0.6 up : -0.728/-0.797
                # iou_up = pseudo_ious > 0.4
                # iou_down = pseudo_ious <= 0.4   #나오는 순서대로 -0.669/-0.757   ##0.5일 때 0.4가 iou_up이었음
                # iou_up2 = pseudo_ious > 0.25 
                # iou_down2 = pseudo_ious <= 0.25 # 얘는 안좋게 나옴 
                # iou_up2 = pseudo_ious > 0.75   #나오는 순서대로 -0.728/-0.806
                # iou_down2 = pseudo_ious <= 0.75
                iou_up2 = pseudo_ious > 0.6   
                iou_down2 = pseudo_ious <= 0.6 # 나오는 순서대로 -0.728/-0.829  -> conf 0.6 up : -0.728/-0.802

                ##################std도 추가
                iou_up = pseudo_ious > 0.5   
                iou_down = pseudo_ious <= 0.5
                
                iou_up2 = pseudo_ious * std_dist > 0.5
                iou_down2 = pseudo_ious * std_dist <= 0.5   #74
                # print(pseudo_ious * std_dist)
                # print(std_dist)
                iou_up3 = pseudo_ious * std_dist > 0.7     #바로 직전 0.6 iou_up4 0.4 iou_up 0.6
                iou_down3 = pseudo_ious * std_dist <= 0.7
                iou_up4 = pseudo_ious * std_dist > 0.3   
                iou_down4 = pseudo_ious * std_dist <= 0.3
                # exit()
                ###################_de4
                ###### ref_length = ref_length / 4 적용
                # iou_up = pseudo_ious > 1
                # iou_down = pseudo_ious <= 1   # 얘가[] 젤 좋음 
                # iou_up2 = pseudo_ious > 1.5   
                # iou_down2 = pseudo_ious <= 1.5  # 0.9랑 성능 똑같 

                # iou_up = pseudo_ious > 0.7
                # iou_down = pseudo_ious <= 0.7
                # iou_up2 = pseudo_ious > 0.9 
                # iou_down2 = pseudo_ious <= 0.9   # 0.7보다는 0.9가 높음 
                ...
                # print(score_up2.sum())  #12 -> 3
                # print(score_up_down2.sum()) #2 -> 0
                # print(score_down_up2.sum()) #51 -> 73
                # print(score_down2.sum()) #19 -> 31
                # print(score_down3.sum())
                # print(score_down4.sum())
                # exit()
            ...
            ###################################################################################
                ...
                #     over_under.append(score_up.sum()/keep.sum())
                # if (score_up_down.sum()/keep.sum()) > 0 :
                #     over_under_2.append(score_up_down.sum()/keep.sum())
                # if (score_down_up.sum()/keep.sum()) > 0 :
                #     over_under_3.append(score_down_up.sum()/keep.sum())
                # if (score_down.sum()/keep.sum()) > 0:
                #     over_under_4.append(score_down.sum()/keep.sum())

                # if score_up2.sum()/keep.sum() > 0:
                #     over_under_5.append(score_up2.sum()/keep.sum())
                # if (score_up_down2.sum()/keep.sum()) > 0 :
                #     over_under_6.append(score_up_down2.sum()/keep.sum())
                # if (score_down_up2.sum()/keep.sum()) > 0 :
                #     over_under_7.append(score_down_up2.sum()/keep.sum())
                # if (score_down2.sum()/keep.sum()) > 0:
                #     over_under_8.append(score_down2.sum()/keep.sum())
               
                # if (score_up.sum()+score_down_up.sum())/keep.sum() > 0 :
                #     over_under_9.append((score_up.sum()+score_down_up.sum())/keep.sum())
                # if (score_down_up.sum()+score_down.sum())/keep.sum() > 0 :
                #     over_under_10.append((score_down_up.sum()+score_down.sum())/keep.sum())      

                # if (score_up2.sum()+score_down_up2.sum())/keep.sum() > 0 :
                #     over_under_11.append((score_up2.sum()+score_down_up2.sum())/keep.sum())
                # if (score_down_up2.sum()+score_down2.sum())/keep.sum() > 0 :
                #     over_under_12.append((score_down_up2.sum()+score_down2.sum())/keep.sum())      
        ...
        result_over_under_10[area_idx] = np.mean(over_under_9).item()
        result_over_under_11[area_idx] = np.mean(over_under_9).item()
        result_over_under_12[area_idx] = np.mean(over_under_9).item()
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_split_distance_ratio_std2/" + str(args.tag1)
