...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                conf_up = conf > 0.5
                conf_down = conf <= 0.5
                iou_up = pseudo_ious > 0.5   # 처음에는 0.65  그 다음은 0.5 : 0.5가 훨씬 좋음 # resize_select할 때는 0.5 -> 0.65 : 0.5가 더 좋음
                iou_down = pseudo_ious <= 0.5 
                dist_up = dists > 0.6 #resize_select 0.6 -> 0.5 : resize_select할 때는 0.6이 좋음
                dist_down = dists <= 0.6
                a = conf_down & iou_up & dist_down  #dist말고 center로 해보기
                b = conf_down & (iou_up | dist_down)
                c = conf_up & iou_down & dist_up
                d = conf_up & (iou_down | dist_up)
                e = conf_up & iou_down & dist_down
                f = conf_up & (iou_down | dist_down)
                if (a.sum()/keep.sum()) > 0 :
                    over_under.append(a.sum()/keep.sum())
                if (b.sum()/keep.sum()) > 0 :
                    over_under_2.append(b.sum()/keep.sum())
                if (c.sum()/keep.sum()) > 0 :
                    over_under_3.append(c.sum()/keep.sum())
                if (d.sum()/keep.sum()) > 0 :
                    over_under_4.append(d.sum()/keep.sum())
                if ((a.sum()+c.sum())/keep.sum()) > 0 :
                    over_under_5.append((a.sum()+c.sum())/keep.sum())
                if ((b.sum()+d.sum())/keep.sum()) > 0 :
                    over_under_6.append((b.sum()+d.sum())/keep.sum())
                if (e.sum()/keep.sum()) > 0 :
                    over_under_7.append(e.sum()/keep.sum())
                if (f.sum()/keep.sum()) > 0 :
                    over_under_8.append(f.sum()/keep.sum())
                # print(a.sum())
                # print(b.sum())
                # print(c.sum())
                # print(d.sum())
                ...
                # # print(1/dists[:25])
                # f_adj = 0.5*pseudo_ious + 0.5/dists  # 젤 좋음 
                # # print(f_adj[:25])
                # iou_up = f_adj > 1.25
                # iou_up = f_adj > 2
                # iou_down = ~iou_up
                # # print(iou_up.sum())
                # f_adj = 0.7*pseudo_ious + 0.3/dists  # 3번째 안 좋음
                # iou_up2 = f_adj > 1.25
                # iou_up2 = f_adj > 2
                # iou_down2 = ~iou_up2
                # # print(iou_up2.sum())
                # f_adj = 0.3*pseudo_ious + 0.7/dists  # 3번째 안 좋음
                # iou_up3 = f_adj > 1.25
                # iou_up3 = f_adj > 2
                # iou_down3 = ~iou_up3
                # # print(iou_up3.sum())
                ...
                # iou_down = pseudo_ious <= 0.5
                # # print(iou_up_ref.sum()) # 76
                # iou_up = iou_up_ref & dist
                # iou_down = iou_down_ref & dist
                # iou_up2 = iou_up_ref & ~dist
                # iou_down2 = iou_down_ref & ~dist
                # print(iou_up.sum())
                # print(iou_up2.sum())
                # print(iou_down.sum())
                # print(iou_down2.sum())
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_split_2_2_all/" + str(args.tag1)
