...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        adj_box_num.append(iou_with_remaining.sum().item())
        removed_box = init_box[iou_with_remaining]
        remove_score = scores[iou_with_remaining] 
        # print(remove_score)
        if iou_with_remaining.sum() == 0 :
            adj_box_conf.append(torch.tensor(0))
        else:    
            adj_box_conf.append(torch.mean(remove_score).item())
        # print(iou_with_remaining.sum())
        # if iou_with_remaining.sum() == 0 :
        #     print("nooooooooooooooooo")
        # print(remove_score)
        # print(torch.mean(remove_score))
        adj_box_conf2.append(torch.sum(remove_score).item())
        # print(torch.sum(remove_score))
        # exit()
        ...
        #     if iou_mean.item() < 0.65:   # 0.65이 베스트 
        #     # if (iou_mean.item() < 0.7) & (inter_over_pseudo_box.item() >= 2/3):
        ...
        #         # 새로운 bbox
        #         pseudo_bbox6 = torch.tensor([new_x1, new_y1, new_x2, new_y2])
        #         pseudo_iou6 = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox6.unsqueeze(0))
            
            
        #         pseudo_ious2.append(pseudo_iou6.item())
        #     else:
        #         pseudo_iou6 = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox.unsqueeze(0))
        #         pseudo_ious2.append(pseudo_iou6.item())
        ...
        #     # if scores[current_idx].item() <= 0.5:
    ...
    return keep, num_boxes, torch.tensor(adj_box_num), torch.tensor(adj_box_conf), torch.tensor(adj_box_conf2)
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep, num_boxes, adj_box_num, adj_box_conf, adj_box_conf2 = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                if conf.shape[0] != keep.sum().item():
                    conf, _ = torch.sort(score_all[keep], descending=True)
                    print("nnnnnnnnnnnnnnnnnnnnnnnnnnn")  # coco 49 에서만 4번 있음
                # print(num_boxes/keep.sum().item()/2)
                # print(conf.shape[0])
                # exit()
                # print(keep.sum().item())
                # exit()
                # print(num_boxes)
                # print(torch.sum(adj_box_num))
                # print(adj_box_num.shape)
                # print(adj_box_conf.shape)
                # exit()
                total_num_boxes = torch.sum(adj_box_num).item()
                # thr = num_boxes/keep.sum().item()/2
                # thr2 = num_boxes/keep.sum().item()
                # thr = (num_boxes-keep.sum())/keep.sum().item()/2
                thr = total_num_boxes/keep.sum().item()/2
                # thr2 = (num_boxes-keep.sum())/keep.sum().item()
                thr2 = total_num_boxes/keep.sum().item()
                no_adj = adj_box_num <= thr
                no_adj2 = adj_box_num <= thr2
                no_adj_ratio = no_adj.sum()/keep.sum()
                no_adj_ratio2 = no_adj2.sum()/keep.sum()

                # print(thr)
                # print(thr2/num_boxes)
                # print(no_adj_ratio)
                # print(no_adj_ratio2)
                # exit()
                # print(conf)
                # print(outputs[3][img_idx])
                # exit()
                conf_up = conf > 0.5
                conf_down = conf <= 0.5
                conf_down_ratio = conf_down.sum()/keep.sum()
                # print(adj_box_num)
                # print(adj_box_num * conf)
                # exit()
                # print(conf_up)
                # print(adj_box_num[conf_down])
                # print(adj_box_num[conf_down].sum()/np.sum(num_boxes))
                # print(adj_box_num[conf_down].sum()/adj_box_num.sum())

                # res_count9
                # num_box_ratio = adj_box_num[conf_up].sum()/np.sum(num_boxes)
                # num_box_ratio2 = adj_box_num[conf_up].sum()/adj_box_num.sum()
                # num_box_ratio3 = adj_box_num[conf_down].sum()/np.sum(num_boxes)
                # num_box_ratio4 = adj_box_num[conf_down].sum()/adj_box_num.sum()
                

                # print(torch.mean(conf))
                # print(num_boxes)
                # print(keep.sum())
                # print(num_boxes/keep.sum())
                # exit()
                #res_count10   : resnet_count11랑 걍 거의 똑같음 하나가 res_count11이 조금 좋음
                num_box_ratio = (adj_box_num*adj_box_conf)[conf_up].sum()/np.sum(num_boxes)  # dl1의 res_count12
                
                # exit()
                # num_box_ratio = (adj_box_num*conf)[conf_up].sum()/(np.sum(num_boxes)*torch.mean(conf))  # brl3 res_count12
                # num_box_ratio = (adj_box_num*conf)[conf_up].sum()/(num_boxes*0.5/keep.sum())  # brl3 res_count13
                # print(num_box_ratio)
                # num_box_ratio2 = (adj_box_num*conf)[conf_up].sum()/keep.sum()
                if adj_box_num.sum().item() != 0:
                    num_box_ratio2 = (adj_box_num*adj_box_conf2)[conf_up].sum()/(adj_box_num*adj_box_conf2).sum()
                    over_under_10.append(num_box_ratio2.item())
                # else:   # 이거 주석처리하는게 res_count11
                    # num_box_ratio2 = torch.tensor(0)
                # num_box_ratio3 = (adj_box_num*conf)[conf_down].sum()/(np.sum(num_boxes)*0.5)
                # num_box_ratio3 = (adj_box_num*conf)[conf_down].sum()/(num_boxes*0.5/keep.sum())
                num_box_ratio3 = (adj_box_num*adj_box_conf)[conf_down].sum()/np.sum(num_boxes)
                if adj_box_num.sum().item() != 0:
                    num_box_ratio4 = (adj_box_num*adj_box_conf2)[conf_down].sum()/(adj_box_num*adj_box_conf2).sum()
                    over_under_12.append(num_box_ratio4.item())
                # else:
                    # num_box_ratio4 = torch.tensor(0)
                # if num_box_ratio.item() > 0:
                #     over_under_7.append(num_box_ratio.item())
                # if num_box_ratio2.item() > 0:
                #     over_under_8.append(num_box_ratio2.item())
                # if num_box_ratio3.item() > 0:
                #     over_under_9.append(num_box_ratio3.item())
                # if num_box_ratio4.item() > 0:
                #     over_under_10.append(num_box_ratio4.item())
                ## res_count7
                # if conf_up.sum() > 0:
                #     over_under_9.append(num_box_ratio.item())
                #     over_under_10.append(num_box_ratio2.item())
                # if conf_down.sum() > 0:
                #     over_under_11.append(num_box_ratio3.item())
                #     over_under_12.append(num_box_ratio4.item())
                ...
                # print(num_box_ratio)
                # print(num_box_ratio2)
                # exit()
                ### 이전 돌린 것에는 이거 주석처리 안 함
                # b = adj_box_num==0
                # adj_box_num[adj_box_num==0] = -1
                
                # print(b.sum())
                # print(conf)
                # print(adj_box_num)
                # print(conf*(adj_box_num/(num_boxes-keep.sum())))
                # print(thr/2*0.5)
                # # print(thr)
                # print(thr/(num_boxes-keep.sum()))
                # cond = conf*(adj_box_num/(num_boxes-keep.sum())) <= thr/(num_boxes-keep.sum())*0.5
                # cond2 = conf*(adj_box_num/(num_boxes-keep.sum())) <= thr2/(num_boxes-keep.sum())*0.5

                # cond = conf*(adj_box_num/num_boxes) <= thr/total_num_boxes*0.5
                # cond2 = conf*(adj_box_num/num_boxes) <= thr2/total_num_boxes*0.5

                cond = (adj_box_num/num_boxes) <= thr2/num_boxes
                cond2 = (adj_box_num/num_boxes) <= thr/num_boxes
                # print(adj_box_num)
                a = conf_down & cond 
                b = conf_down & cond2
                # print(conf*(adj_box_num/total_num_boxes))
                # print(thr/total_num_boxes*0.5)
                # exit()
                # print(2/num_boxes*0.5)
                # print(no_adj_ratio)
                # print(no_adj_ratio2)
                # print(a.sum()/keep.sum())
                # print(b.sum()/keep.sum())
                # exit()
                # print(no_adj_ratio)
                # print(no_adj_ratio2)
                # print(no_adj_ratio*conf_down_ratio)
                # print(no_adj_ratio2*conf_down_ratio)
                # if no_adj_ratio.item() > 0:
                #     over_under.append(no_adj_ratio.item())
                # if no_adj_ratio2.item() > 0:
                #     over_under_2.append(no_adj_ratio.item())
                # if no_adj_ratio*conf_down_ratio.item() > 0:
                #     over_under_3.append(no_adj_ratio*conf_down_ratio.item())
                # if no_adj_ratio2*conf_down_ratio.item() > 0:
                #     over_under_4.append(no_adj_ratio2*conf_down_ratio.item())
                # if a.sum()/keep.sum() > 0 :
                #     over_under_5.append(a.sum()/keep.sum())
                # if b.sum()/keep.sum() > 0 :
                #     over_under_6.append(b.sum()/keep.sum())
                conf_up = conf > 0.4
                conf_down = conf <= 0.4
                ...
                #res_count10
                # num_box_ratio = (adj_box_num*conf)[conf_up].sum()/(num_boxes*0.5/keep.sum())
                num_box_ratio = (adj_box_num*adj_box_conf)[conf_up].sum()/np.sum(num_boxes) 
                # num_box_ratio2 = (adj_box_num*conf)[conf_up].sum()/keep.sum()
                if adj_box_num.sum().item() != 0:
                    num_box_ratio2 = (adj_box_num*adj_box_conf2)[conf_up].sum()/(adj_box_num*adj_box_conf2).sum()
                    over_under_2.append(num_box_ratio2.item())
                # else:
                    # num_box_ratio2 = torch.tensor(0)
                # num_box_ratio3 = (adj_box_num*conf)[conf_down].sum()/(num_boxes*0.5/keep.sum())
                num_box_ratio3 = (adj_box_num*adj_box_conf)[conf_down].sum()/np.sum(num_boxes)
                ...
                if adj_box_num.sum().item() != 0:
                    num_box_ratio4 = (adj_box_num*adj_box_conf2)[conf_down].sum()/(adj_box_num*adj_box_conf2).sum()
                    over_under_4.append(num_box_ratio4.item())
                # else:
                    # num_box_ratio4 = torch.tensor(0)
                # if conf_up.sum() > 0:
                #     over_under.append(num_box_ratio.item())
                #     over_under_2.append(num_box_ratio2.item())
                # if conf_down.sum() > 0:
                #     over_under_3.append(num_box_ratio3.item())
                #     over_under_4.append(num_box_ratio4.item())
                ...
                if adj_box_num.sum().item() != 0:
                    num_box_ratio2 = (adj_box_num*adj_box_conf2)[conf_up].sum()/(adj_box_num*adj_box_conf2).sum()
                    over_under_6.append(num_box_ratio2.item())
                # else:    # res_Count11 은 이거 없이
                #     num_box_ratio2 = torch.tensor(0)
                # num_box_ratio3 = (adj_box_num*conf)[conf_down].sum()/(num_boxes*0.5/keep.sum())
                num_box_ratio3 = (adj_box_num*adj_box_conf)[conf_down].sum()/np.sum(num_boxes)
                if adj_box_num.sum().item() != 0:
                    num_box_ratio4 = (adj_box_num*adj_box_conf2)[conf_down].sum()/(adj_box_num*adj_box_conf2).sum()
                    over_under_8.append(num_box_ratio4.item())
                # else:
                    # num_box_ratio4 = torch.tensor(0)

                
                # if conf_up.sum() > 0:
                #     over_under_5.append(num_box_ratio.item())
                #     over_under_6.append(num_box_ratio2.item())
                # if conf_down.sum() > 0:
                #     over_under_7.append(num_box_ratio3.item())
                #     over_under_8.append(num_box_ratio4.item())
                ...
                # # print(no)
                # # exit()
                # if ((score_down_up.sum()+no)/keep.sum()) > 0 :
                #     over_under.append(score_down_up.sum()/keep.sum())
                # if (score_down_up.sum()+score_up.sum())/keep.sum() > 0:
                #     over_under_2.append((score_down_up.sum()+score_up.sum())/keep.sum())
                # if (score_down_up.sum()+score_down.sum())/keep.sum() > 0:
                ...
                # if ((score_down_up2.sum()+no)/keep.sum()) > 0 :
                #     over_under_4.append(score_down_up2.sum()/keep.sum())
                # if (score_down_up2.sum()+score_up2.sum())/keep.sum() > 0:
                #     over_under_5.append((score_down_up2.sum()+score_up2.sum())/keep.sum())
                # if (score_down_up2.sum()+score_down2.sum())/keep.sum() > 0:
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_count15/" + str(args.tag1)
