...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if len(remove_score) > 0:
            remove_iou = iou_values_all[current_idx][remove_idx]
            if len(remove_score) == 1: 
                # over_under += torch.sqrt(((remove_score-sorted_scores[i])/remove_iou)**2).item()  # 얘가 더 좋음ㅁ
                # over_under += torch.abs(remove_score-sorted_scores[i]).item()
                over_under += torch.sqrt(((remove_score-sorted_scores[i])**2)/remove_iou).item()
                over_under2 += torch.sqrt(((remove_score-sorted_scores[i])**2)/remove_iou).item()
                # print(over_under)
            else :
                # over_under += torch.std((remove_score-sorted_scores[i])/remove_iou)  
                # print(over_under)
                # over_under += torch.abs(torch.mean(remove_score)-sorted_scores[i])
                over_under += torch.std(remove_score-sorted_scores[i])/torch.std(remove_iou)  
                over_under2 += (torch.std(remove_score-sorted_scores[i])/torch.std(remove_iou))/len(remove_score)
    # Return indices of boxes that are kept
    kept_indices = sorted_indices[keep.bool()]
    # print("num", num)
    # exit()
    # return kept_indices, remove_bbox
    return keep, over_under, over_under2
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            iou_out_ex = torch.tensor(np.array(outputs[8][img_idx]))
            # score_out_ex = torch.tensor(np.array(outputs[3][img_idx]))  # score_ex랑 똑같음 
            iou_out_ex_o = torch.tensor(np.array(outputs[8][0]))
            if len(iou_all) > 0 :
                ...
                keep, under_over_num, under_over_num2 = apply_iou_filter(score_all, iou_all, 0.5)

                # iou_ex = iou_all[keep.bool(), :][:, keep.bool()]  # outputs[8][img_idx] 이거랑 안 같음 내 추측은 outputs[8][img_idx]은 score 크기 순으로 sort한 값 같음 (mean한 값이 같은 거 보니 추측이 맞는 듯)

                # # iou_ex = iou_exs[img_idx]
                # iou_ex_2 = iou_all[keep.bool(), :]
                ...
                over_under_3.append(under_over_num2)
                if len(iou_all)-keep.sum() > 0 :
                    over_under_2.append(under_over_num/(len(iou_all)-keep.sum()))
    ...
    if rank == 0:
        ...
        if args.eval:
            ...
            if len(outputs) >= 4:
                metric = dataset.evaluate(outputs[0], iou_out_ex_o,  **eval_kwargs)  # scores: (10, 101, 1, 4, 3)  precision_score_zero: (10, 1, 4, 3) 
                min_recall = 0
            else:
                metric = dataset.evaluate(outputs, iou_out_ex_o, **eval_kwargs)  # scores: (10, 101, 1, 4, 3)
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_0923_4/" + str(args.tag1)
            ...
            with open(NAME, "r", encoding="utf-8") as f:
                ...
                if data == {}:
                    data.update({0: [[metric['bbox_mAP'], metric['bbox_mAP_50'], metric['bbox_mAP_75'], metric['bbox_mAP_s'], metric['bbox_mAP_m'], metric['bbox_mAP_l']], least_cost, \
                        least_iou_cost_final,  result_over_under, result_over_under_2, result_over_under_3, least_reg_cost_final, least_cls_cost_final, num_cls_large_k, mean_cls_large_k, results_cls_entropy_small_k]})
                else:
