...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                # conf_adj = conf - 0.1*reg2  # 그 전 02.18은 0.1이었음음
                ...
                # print((count_soft.sum()/keep.sum())/num_box_ratio)
                # exit()
                # if num_box_ratio.item() == 0:
                #     print(num_box_ratio+ EPSILON) 
                #     exit()
    ...
    if rank == 0:
        ...
        #원래는 no conf
            NAME_DIR  = "./02.21/r50_retina_CCC/" + str(args.tag1)  
