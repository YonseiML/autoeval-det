...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            pseudo_bbox2 = torch.tensor([x1_min, y1_min, x2_max, y2_max])
            ...
            # if (iou_mean.item() < 0.7) & (inter_over_pseudo_box.item() == 1):
            # if (iou_mean.item() < 0.7) & (iou_mean2.item() < 0.7) & (inter_over_pseudo_box.item() >= 2/3):
                ...
                scale_factor2 = target_width2 / original_width
                new_width2 = target_width2
                new_height = original_height * scale_factor
                new_height2 = original_height * scale_factor2
                ...
                new_x_min2 = center_x - new_width2 / 2
                new_y_min2 = center_y - new_height2 / 2
                new_x_max2 = center_x + new_width2 / 2
                new_y_max2 = center_y + new_height2 / 2

                # 결과 bbox 반환
                pseudo_bbox = torch.tensor([new_x_min, new_y_min, new_x_max, new_y_max])
                pseudo_bbox2 = torch.tensor([new_x_min2, new_y_min2, new_x_max2, new_y_max2])
                ######ori#####
            ...
            pseudo_iou = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox.unsqueeze(0))
            pseudo_iou2 = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox2.unsqueeze(0))
            pseudo_ious.append(pseudo_iou.item())
            pseudo_ious2.append(pseudo_iou2.item())
...
def main():
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_split_ablation2/" + str(args.tag1)
