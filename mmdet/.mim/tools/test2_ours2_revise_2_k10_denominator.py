...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        # exit()
        #이게 brl3임 밑은 dl1
        # agnostic_nms = (remove_score >= 0.8).sum() * 1 + ((remove_score < 0.8)&(remove_score>=0.6)).sum() * 1 + (remove_score < 0.6).sum() * 0.5
        # agnostic_nms = (remove_score >= 0.8).sum() * 1 + ((remove_score < 0.8)&(remove_score>=0.6)).sum() * 0.8 + (remove_score < 0.6).sum() * 0.6
        # sotf version
        # print(remove_score)
        # print(1 / (1 + np.exp(-10 * (remove_score - 0.5))))
        # agnostic_nms = (1 / (1 + np.exp(-5 * (remove_score - 0.5)))).sum()

        agnostic_nms = (0.5 + 0.5 / (1 + np.exp(-10 * (remove_score - 0.6)))).sum()
        # agnostic_nms = remove_score.sum()
        # print(remove_score.sum())
        # print(agnostic_nms)
        # exit()
        # 1 / (1 + np.exp(-k * (x - 0.5)))
        adj_box_confup_num.append(agnostic_nms.item())
        # adj_box_confup_num.append((agnostic_nms/iou_with_remaining.sum()).item())  # 더 안 좋음
        # adj_box_confup_num.append((remove_score > 0.07).sum().item())
        adj_box_conf.append(torch.sum(remove_score).item())
        # exit()
    ...
    # return keep, num_boxes, torch.tensor(adj_box_num), torch.tensor(adj_box_conf)
    return keep, num_boxes, torch.tensor(adj_box_confup_num), torch.tensor(adj_box_conf) 
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep, num_boxes, adj_box_num, adj_box_conf = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                #brl3 밑은 dl1
                # under = (score_all >= 0.8).sum() * 1 + ((score_all < 0.8)&(score_all>=0.6)).sum() * 1 + (score_all < 0.6).sum() * 0.5
                # under = (score_all >= 0.8).sum() * 1 + ((score_all < 0.8)&(score_all>=0.6)).sum() * 0.8 + (score_all < 0.6).sum() * 0.6
                #soft version
                under = (0.5 + 0.5 / (1 + np.exp(-10 * (score_all - 0.6)))).sum()
                # under = adj_box_num.sum()
                ...
                num_box_ratio3 = adj_box_num[conf_down].sum()/under
                if adj_box_num.sum().item() != 0:
                ...
                '''
                if adj_box_num.sum().item() != 0:
                    num_box_ratio = (adj_box_num/num_boxes)[conf_up].sum()/(adj_box_num/num_boxes).sum()
                    over_under_9.append(num_box_ratio.item())
                if adj_box_conf.sum().item() != 0:
                    num_box_ratio2 = (adj_box_conf[conf_up]).sum()/adj_box_conf.sum()
                    over_under_10.append(num_box_ratio2.item())
                
                if adj_box_num.sum().item() != 0:
                    num_box_ratio3 = (adj_box_num/num_boxes)[conf_down].sum()/(adj_box_num/num_boxes).sum()
                    over_under_11.append(num_box_ratio.item())
                if adj_box_conf.sum().item() != 0:
                    num_box_ratio4 = (adj_box_conf[conf_down]).sum()/adj_box_conf.sum()
                    over_under_12.append(num_box_ratio2.item())
                ...
                if adj_box_num.sum().item() != 0:
                    num_box_ratio3 = (adj_box_num/num_boxes)[conf_down].sum()/(adj_box_num/num_boxes).sum()
                    over_under_3.append(num_box_ratio.item())
                if adj_box_conf.sum().item() != 0:
                    num_box_ratio4 = (adj_box_conf[conf_down]).sum()/adj_box_conf.sum()
                    over_under_4.append(num_box_ratio2.item())
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_ours2_revise_2_k10_denominator/" + str(args.tag1)
