...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            if ref <= 0.5 :
                ref_iou = ref_iou.squeeze(0)
                high_conf_high_iou = ref_iou[removed_score > ref] > 0.7
                low_conf_low_iou = ref_iou[removed_score <= ref] <= 0.7
                score += ((high_conf_high_iou.sum()+low_conf_low_iou.sum())/removed_score.shape[0])
                # if (high_conf_high_iou.sum()+low_conf_low_iou.sum())/removed_score.shape[0] >= 0.3:
                #     score += 1
                # print((high_conf_high_iou.sum()+low_conf_low_iou.sum())/removed_score.shape[0])
        # num_adj += removed_score.shape[0]
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep, final = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                # exit()
                # print(final/keep.sum())
                # exit()
                if final.item() > 0:
                    over_under.append(final.item())
