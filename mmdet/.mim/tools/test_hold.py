...
def apply_iou_filter(scores: torch.Tensor,
    ...
    # print(keep.sum())
    # exit()
    predicted_box = init_box[keep]
    predicted_iou = torchvision.ops.box_iou(predicted_box , predicted_box)
    # print(predicted_iou.shape)
    diag_mask = torch.eye(predicted_iou.size(0), dtype=torch.bool)
    predicted_iou = predicted_iou[~diag_mask].view(predicted_iou.size(0), -1)
    # print(predicted_iou.shape)
    # print(predicted_iou[0])
    adj_predicted = predicted_iou > 0.3
    # print(torch.sum(adj_predicted, dim=1))
    adj_predicted_num = torch.sum(adj_predicted, dim=1)
    # exit()
    # return kept_indices, remove_bbox
    return keep, num_boxes, torch.tensor(adj_box_num), adj_predicted_num
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep, num_boxes, adj_box_num, adj_predicted_num = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                # print(adj_predicted_num)
                adj_predicted_num[adj_predicted_num==0] = 1
                # print(adj_box_num)
                # print(adj_box_num/adj_predicted_num)
                # exit()

                
                _, ref_sort_idx = torch.sort(conf, descending=True)
                _, sort_idx = torch.sort(adj_box_num, descending=True)
                # print(sort_idx)
                _, sort_idx = torch.sort(adj_box_num/adj_predicted_num, descending=True)
                # print(sort_idx)
                # exit()
                tensor_min = ref_sort_idx.min()
                tensor_max = ref_sort_idx.max()

                # 정규화
                ref_sort_idx = (ref_sort_idx - tensor_min) / (tensor_max - tensor_min)
                tensor_min = sort_idx.min()
                tensor_max = sort_idx.max()
                sort_idx = (sort_idx - tensor_min) / (tensor_max - tensor_min)
                # print(sort_idx)
                # print(ref_sort_idx)
                # print(ref_sort_idx-sort_idx)
                # print(torch.norm(ref_sort_idx.float() - sort_idx.float(), p=2))
                diff = torch.norm(ref_sort_idx.float() - sort_idx.float(), p=2)
                # exit()
                if diff.item() > 0:
                    over_under.append(diff.item())
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_count4/" + str(args.tag1)
            ...
            with open(NAME, "r", encoding="utf-8") as f:
                ...
                if data == {}:
                    data.update({0: [[metric['bbox_mAP'], metric['bbox_mAP_50'], metric['bbox_mAP_75'], metric['bbox_mAP_s'], metric['bbox_mAP_m'], metric['bbox_mAP_l']], least_cost, \
                        least_iou_cost_final,  result_over_under, result_over_under_2, result_over_under_3, result_over_under_4, result_over_under_5, result_over_under_6, result_over_under_7, result_over_under_8, result_over_under_9, least_reg_cost_final, least_cls_cost_final, num_cls_large_k, mean_cls_large_k, results_cls_entropy_small_k]})
                else:
