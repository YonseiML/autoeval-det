...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            #### iou high ####
            removed_iou = iou_values_all[current_idx][iou_with_remaining]
            iou_high = torch.max(removed_iou)
            pseudo_ious.append(iou_high.item())

            #### conf high ####
            remove_score = scores[iou_with_remaining] 
            conf_high = torch.max(remove_score)
            pseudo_ious2.append(conf_high.item())
...
def main():
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_split_only_high/" + str(args.tag1)
