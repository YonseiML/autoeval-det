...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        # removed_box = init_box[remove_idx.bool()]

        
        # ref_iou = iou_values_all[current_idx][remove_idx.bool()]
        # print(inter_over_pseudo_box)
        # exit()
        ...
        if ref <= 0.5 :
            num_adj.append(removed_box.shape[0])
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            if ref > 0.5 :
                ...
                iou_up8 = ref_iou > 0.65
                iou_down8 = ref_iou <= 0.65
    ...
    exit()
    score = (torch.tensor(num_adj) >= (keep.sum()/4)).sum()
    score2 = (torch.tensor(num_adj) >= (keep.sum()/5)).sum()
    ...
    return keep, score7, score8, score9, nothing, score, score2
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep, score7, score8, score9, nothing, score, score2 = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                # denominator = (score_all.shape[0] - keep.sum())
                # denominator = score_up7+score_up_down7+score_down_up7+score_down7 + nothing
                denominator = score_up7+score_up_down7+score_down_up7+score_down7
                # print(denominator)
                # print(score_up7+score_up_down7+score_down_up7+score_down7)
                # print(score_up8+score_up_down8+score_down_up8+score_down8)
                # print(score_up9+score_up_down9+score_down_up9+score_down9)
                # print(score_down_up7/denominator)
                # print(score/keep.sum())
                # exit()
                if score/keep.sum() :
                    over_under.append(score/keep.sum())
                if score2/keep.sum() :
                    over_under_2.append(score2/keep.sum())
                # if score_up7/denominator > 0:
                #     over_under.append(score_up7/denominator)
                # if score_up_down7/denominator > 0:
                #     over_under_2.append(score_up_down7/denominator)
                if score_down_up7/denominator > 0:
                    over_under_3.append(score_down_up7/denominator)
                if score_down7/denominator > 0:
                    over_under_4.append(score_down7/denominator)

                if score_up8/denominator > 0:
                    over_under_5.append(score_up8/denominator)
                if score_up_down8/denominator > 0:
                    over_under_6.append(score_up_down8/denominator)
                if score_down_up8/denominator > 0:
                    over_under_7.append(score_down_up8/denominator)
                if score_down8/denominator > 0:
                    over_under_8.append(score_down8/denominator)
                
                if score_up9/denominator > 0:
                    over_under_9.append(score_up9/denominator)
                if score_up_down9/denominator > 0:
                    over_under_10.append(score_up_down9/denominator)
                if score_down_up9/denominator > 0:
                    over_under_11.append(score_down_up9/denominator)
                if score_down9/denominator > 0:
                    over_under_12.append(score_down9/denominator)
