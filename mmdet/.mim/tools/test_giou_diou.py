    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            if iou_mean.item() < 0.7:
                ...
                enclosure_area = (new_x2 - new_x1) * (new_y2 - new_y1)
                diagonal_length = (new_x2 - new_x1)**2 + (new_y2 - new_y1)**2
            x1 = torch.max(pseudo_bbox[0], ref_bbox[0])
            y1 = torch.max(pseudo_bbox[1], ref_bbox[1])
            x2 = torch.min(pseudo_bbox[2], ref_bbox[2])
            y2 = torch.min(pseudo_bbox[3], ref_bbox[3])
            inter_w = (x2 - x1).clamp(min=0)
            inter_h = (y2 - y1).clamp(min=0)
            
            # 교집합 면적
            intersection = inter_w * inter_h
            iou = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox.unsqueeze(0))
            union = (torchvision.ops.box_area(init_box[current_idx].unsqueeze(0)) + torchvision.ops.box_area(pseudo_bbox.unsqueeze(0)) - intersection)
            ...
            pseudo_ious.append(giou.item())
            center1_x = (pseudo_bbox[0] + ref_bbox[2]) / 2
            center1_y = (pseudo_bbox[1] + ref_bbox[3]) / 2
            center2_x = (pseudo_bbox[0] + ref_bbox[2]) / 2
            center2_y = (pseudo_bbox[1] + ref_bbox[3]) / 2
            center_distance = (center1_x - center2_x)**2 + (center1_y - center2_y)**2
            diou = iou - (center_distance / diagonal_length)
            pseudo_ious2.append(diou)
        else:
            no += 1
...
def main():
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_split_giou_diou/" + str(args.tag1)
