...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            img_width = 3900 #1333 # cityscapes 2200  # exdark 3900
            img_height = 3000 #800 # cityscapes 1320  # exdark 3000
            init_norm_box = torch.zeros_like(removed_box)
            # print(init_norm_box.shape)
            init_norm_box[:, 0] = removed_box[:, 0] / img_width  # x_min 좌표 정규화
            init_norm_box[:, 1] = removed_box[:, 1] / img_height  # y_min 좌표 정규화
            init_norm_box[:, 2] = removed_box[:, 2] / img_width  # x_max 좌표 정규화
            init_norm_box[:, 3] = removed_box[:, 3] / img_height  # y_max 좌표 정규화
            ref_box[0] = ref_box[0] / img_width  # x_min 좌표 정규화
            ref_box[1] = ref_box[1] / img_height  # y_min 좌표 정규화
            ref_box[2] = ref_box[2] / img_width  # x_max 좌표 정규화
            ref_box[3] = ref_box[3] / img_height  # y_max 좌표 정규화
            labels = np.ones(init_norm_box.size(0))
            wfb_boxes, wfb_scores, label = weighted_boxes_fusion([init_norm_box.tolist()], [removed_score.tolist()], [labels], iou_thr=0.5, skip_box_thr=0.0001, conf_type='avg',)
            # print(wfb_boxes.shape)  # 하나 이상의 box가 생길 수 있음 iou thr를 넘지 못한 box가 있을 경우
            # print(torch.tensor(wfb_boxes))
            pseudo_iou = torchvision.ops.box_iou(ref_box.unsqueeze(0) , torch.tensor(wfb_boxes))
            iou.append(torch.max(pseudo_iou).item())
            
        else:
            iou.append(0)
    
      
    return keep, torch.tensor(iou)
...
def main():
    ...
    if not distributed:
        ...
        # results_cls_per_dataset_ori, results_cls_per_img_ori, iou_alls, iou_exs = outputs[5:]
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ##########전체에서 wbf하는 방법##############
            '''
            score_all = results_cls_per_img_ori[img_idx]
            init_box = torch.tensor(np.array(init_boxes[img_idx]))
            img_width = 1333
            img_height = 800
            init_norm_box = torch.zeros_like(torch.tensor(init_box))
            init_norm_box[:, 0] = init_box[:, 0] / img_width  # x_min 좌표 정규화
            init_norm_box[:, 1] = init_box[:, 1] / img_height  # y_min 좌표 정규화
            init_norm_box[:, 2] = init_box[:, 2] / img_width  # x_max 좌표 정규화
            init_norm_box[:, 3] = init_box[:, 3] / img_height  # y_max 좌표 정규화
            labels = np.ones(len(score_all))
            
            wfb_boxes, scores, label = weighted_boxes_fusion([init_norm_box.tolist()], [score_all], [labels], iou_thr=0.5, skip_box_thr=0.0001, conf_type='avg',)
            '''
            ##############################################3
            ...
            if len(iou_all) > 0 :
                ...
                keep, pseudo_ious = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                if (score_up_down.sum()+score_down_up.sum()+score_down.sum())/keep.sum() > 0:
                    over_under_8.append((score_up_down.sum()+score_down_up.sum()+score_down.sum())/keep.sum())

                ############ res_1007_3 ####################conf_0.6에는 없음
                if (score_down_up.sum()/(keep.sum()-score_down.sum())) > 0 :
                    over_under_9.append(score_down_up.sum()/(keep.sum()-score_down.sum()))   
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_1007_wbf/" + str(args.tag1)
