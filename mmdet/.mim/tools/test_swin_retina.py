...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            # model_score1 = abs(abs(predicted_score-remove_score)-(1-pre_iou.squeeze(0))) #sr3  # 얘가 안 좋음
            model_score1 = abs(predicted_score-remove_score)-(1-pre_iou.squeeze(0))  # sr2
            ...
            # pseudo_ious2.append((distance/ref_dist).item())
    ...
    return keep, torch.tensor(pseudo_ious), torch.tensor(pseudo_ious2), torch.tensor(reg2), torch.tensor(adj_box_confup_num)
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep, pseudo_ious, pseudo_ious2, reg2, adj_box_num = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                # conf_up = conf > 0.7
                # conf_down = conf <= 0.7
                # # conf_adj = conf-0.7*reg2
                # # conf_up = conf_adj > 0.7
                # # conf_down = conf_adj <= 0.7
                ...
                # # pseudo_ious2 = 1 / (1 + np.exp(-10 * (pseudo_ious2 - 0.5)))  ## retinanet4  # 5랑 별 차이를...
                # pseudo_ious2 = 1 / (1 + np.exp(-20 * (pseudo_ious2 - 0.6)))  # retinanet6  
                # # pseudo_ious2 = 1 / (1 + np.exp(-100 * (pseudo_ious2 - 0.5)))  # retinanet5
                # com_iou = pseudo_ious
                ...
                # # 4부터 추가
                # com_iou8 = pseudo_ious + 0.5*pseudo_ious2
                # com_iou9 = 0.5*pseudo_ious + 0.5*pseudo_ious2
                ...
                # conf1  = 1 / (1 + np.exp(100 * (conf1 - 0.5)))
                # conf2 = conf-0.6*torch.mean(reg)
                # conf2  = 1 / (1 + np.exp(100 * (conf2 - 0.5)))
                # conf3 = conf-0.7*torch.mean(reg2.float())
                # conf3  = 1 / (1 + np.exp(100 * (conf3 - 0.5)))
                # conf4 = conf-0.6*torch.mean(reg2.float())
                # conf4  = 1 / (1 + np.exp(100 * (conf4 - 0.5)))
                # conf_ori = 1 / (1 + np.exp(100 * (conf - 0.5)))
                ...
                # count_soft5 = (conf2+com_iou4)/2
                # count_soft6 = (conf2+com_iou5)/2
                # count_soft7 = (conf1+com_iou8)/2
                # count_soft8 = (conf1+com_iou9)/2
                # count_soft9 = (conf2+com_iou8)/2
                # count_soft10 = (conf2+com_iou9)/2
                # count_ori = (conf_ori+com_iou4)/2
                # count_ori2 = (conf_ori+com_iou8)/2
                ...
                # ####4이후부터 추가 
                # if count_ori.sum()/keep.sum() > 0:
                #     over_under_11.append(count_ori.sum()/keep.sum())
                # if count_ori2.sum()/keep.sum() > 0:
                #     over_under_12.append(count_ori2.sum()/keep.sum())
                ...
                com_iou2 = 1 / (1 + np.exp(-100 * (com_iou1 - 0.4)))
                com_iou3 = 1 / (1 + np.exp(-100 * (com_iou1 - 0.3)))
                com_iou4 = 1 / (1 + np.exp(-100 * (com_iou1 - 0.6)))
                com_iou5 = 1 / (1 + np.exp(-100 * (com_iou1 - 0.7)))
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./02.23/ablation_conf_thr0.7_swin_retina/" + str(args.tag1)
