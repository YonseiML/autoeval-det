...
def apply_iou_filter(scores: torch.Tensor,
    ...
    # print(iou)
    # print(pseudo_ious)
    score = torch.tensor(pseudo_ious) <= 0.6
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            init_box = torch.tensor(init_boxes[img_idx])
            ...
            if len(iou_all) > 0 :
                ...
                # keep, score = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                
                if torch.std(score_ex)> 0 :
                    over_under.append(torch.std(score_ex)) # 원래 score의 std
                if torch.std(score_all)> 0 :
                    over_under_2.append(torch.std(score_all)) # 원래 score의 std
                if torch.mean(score_ex)> 0 :
                    over_under_3.append(torch.mean(score_ex)) # 원래 score의 std
                if torch.mean(score_all)> 0 :
                    over_under_4.append(torch.mean(score_all)) # 원래 score의 std
            # exit()
            #####################################################
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_base_modi/" + str(args.tag1)
            ...
            with open(NAME, "r", encoding="utf-8") as f:
                ...
                if data == {}:
                    data.update({0: [[metric['bbox_mAP'], metric['bbox_mAP_50'], metric['bbox_mAP_75'], metric['bbox_mAP_s'], metric['bbox_mAP_m'], metric['bbox_mAP_l']], least_cost, \
                        least_iou_cost_final,  result_over_under, result_over_under_2, result_over_under_3, result_over_under_4, least_reg_cost_final, least_cls_cost_final, num_cls_large_k, mean_cls_large_k, results_cls_entropy_small_k]})
                else:
