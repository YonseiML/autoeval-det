...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            # weigt average로 predicted score 내보내기
            # height, width, x, y of center
            # iou, distance    
        else:
            no += 1
    ...
    return keep, torch.tensor(pseudo_ious), torch.tensor(dists)
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep, pseudo_ious, dists = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                f_adj = 0.3*pseudo_ious + 0.3/dists + 0.4*conf
                f_adj2 = 0.4*pseudo_ious + 0.6*conf
                f_adj3 = 0.5*pseudo_ious + 0.5*conf
                conf_down = conf <= 0.5
                iou_up = pseudo_ious > 0.5
                a = f_adj[conf_down]
                b = f_adj[conf_down & iou_up]
                c = f_adj2[conf_down]
                d = f_adj2[conf_down & iou_up]
                e = f_adj3[conf_down]
                f = f_adj3[conf_down & iou_up]
                if a.shape[0] >= 1:
                    over_under.append(torch.mean(a).item())
                if b.shape[0] >= 1:
                    over_under_2.append(torch.mean(b).item())
                if c.shape[0] >= 1:
                    over_under_3.append(torch.mean(c).item())
                if d.shape[0] >= 1:
                    over_under_4.append(torch.mean(d).item())
                if e.shape[0] >= 1:
                    over_under_5.append(torch.mean(e).item())
                if f.shape[0] >= 1:
                    over_under_6.append(torch.mean(f).item())    
                
                # print(f_adj)
                # print(f_adj2)
                # print(f_adj3)
                
                # print(pseudo_ious[:25])
                # print(1/dists[:25])
                # f_adj = 0.5*pseudo_ious + 0.5/dists




                # f_adj = pseudo_ious/dists
                # f_adj2 = dists/pseudo_ious
                # iou_up = f_adj > 1
                # iou_up2 = f_adj2 <= 1
                # conf_down = conf <= 0.5
                # score_down_up = conf_down & iou_up
                # score_down_up2 = conf_down & iou_up2
                # # print(score_down_up.sum())
                # f_adj3 = 0.5*pseudo_ious + 0.5/dists 
                # iou_up3 = f_adj > 0.5
                # score_down_up3 = conf_down & iou_up3
                # # print(score_down_up3.sum())
                # # print(score_down_up2.sum())
                # # exit()
                # ############res_split_weight_2_no_split이랑 2는 아래의 방법으로 진행###########  : 이때 실수 conf_down으로 해야하는데 conf_up으로 실험 진행
                # # f_adj = 0.5*pseudo_ious + 0.5*dists  # 얘랑 f_adj2가 성능 비슷하고 f_adj3은 성능 바닥
                # f_adj2 = 0.5*pseudo_ious + 0.5/dists
                # # # print(f_adj[:25])
                # iou_up2 = f_adj2 <= 0.75
                # score_down_up2 = conf_down & iou_up2
                # # iou_up = f_adj > 1.25
                # # iou_down = ~iou_up
                # # # print(iou_up.sum())
                # # f_adj2 = 0.7*pseudo_ious + 0.3/dists  # dists를 곱하는 것보다 나누는 것이 성능이 좋음 
                # f_adj = 0.7*pseudo_ious + 0.3/dists
                # iou_up = f_adj > 0.75
                # score_down_up = conf_down & iou_up
                ...
                # # f_adj2 = 0.7*pseudo_ious + 0.3*dists  
                # iou_up2 = f_adj2 > 1.25
                # iou_down2 = ~iou_up2
                # # print(iou_up2.sum())
                # f_adj3 = 0.3*pseudo_ious + 0.7/dists
                # # f_adj3 = 0.3*pseudo_ious + 0.7*dists
                # iou_up3 = f_adj3 > 1.25
                # iou_down3 = ~iou_up3
                # # print(iou_up3.sum())
                ...
                # print(score_up.sum())  #3
                # print(score_up_down.sum()) #0
                # print(score_down_up.sum()) #73
                # print(score_down.sum()) #31
                # print(score_up2.sum())  #3
                # print(score_up_down2.sum()) #0
                # print(score_down_up2.sum()) #73
                # print(score_down2.sum()) #31
                # print(score_up3.sum())  #3
                # print(score_up_down3.sum()) #0
                # print(score_down_up3.sum()) #73
                # print(score_down3.sum()) #31
                # print(keep.sum()) #107
                # exit()
                ...
                #     over_under_2.append(score_up.sum()/keep.sum())
                # if (score_up_down.sum()/keep.sum()) > 0 :
                #     over_under_3.append(score_up_down.sum()/keep.sum())
                ...
                #     over_under_7.append(score_down2.sum()/keep.sum())  
                # if (score_down_up2.sum()/keep.sum()) > 0 :
                #     over_under_8.append(score_down_up2.sum()/keep.sum())
                
                # if (score_down_up3.sum()/keep.sum()) > 0 :
                #     over_under_9.append(score_down_up3.sum()/keep.sum())
                # if (score_up3.sum()/keep.sum()) > 0 :
                #     over_under_10.append(score_up3.sum()/keep.sum())
                # if (score_up_down3.sum()/keep.sum()) > 0 :
                #     over_under_11.append(score_up_down3.sum()/keep.sum())
                # if (score_down3.sum()/keep.sum()) > 0 :
                #     over_under_12.append(score_down3.sum()/keep.sum())



                # if f_adj[score_down_up].shape[0] >= 1:
                #     over_under.append(torch.mean(f_adj[score_down_up]).item())
                # if f_adj[conf_down].shape[0] >= 1:
                #     over_under_2.append(torch.mean(f_adj[conf_down]).item())    # 이거 왜 conf_up으로 했지???
                #     # print(over_under_2)   

                # if f_adj2[score_down_up2].shape[0] >= 1:     # 0.402/0.442
                #     over_under_3.append(torch.mean(f_adj2[score_down_up2]).item())
                #     # print(over_under_3)
                # if f_adj2[conf_down].shape[0] >= 1:
                #     over_under_4.append(torch.mean(f_adj2[conf_down]).item())
                #     # print(over_under_4)   

                # if f_adj3[score_down_up3].shape[0] >= 1:
                #     over_under_5.append(torch.mean(f_adj3[score_down_up3]).item())
                #     # print(over_under_5)
                # if f_adj3[conf_down].shape[0] >= 1:
                #     over_under_6.append(torch.mean(f_adj3[conf_down]).item())
                    # print(over_under_6)   
                # exit()    
            #####################################################
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_split_weight_3/" + str(args.tag1)
