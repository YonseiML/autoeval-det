...
def main():
    ...
    result_ours = [0 for area_idx in areaRng]
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                if score_down.sum()/keep.sum() > 0:
                    ours.append(score_down.sum()/keep.sum())
        ...
        result_ours[area_idx] = np.mean(ours).item() 
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            # test 
            # NAME_DIR  = f"./res_multiweather_test_model/{args.expert}_expert/{str(args.tag1)}/" 
            # if not os.path.exists(NAME_DIR):
            #     os.makedirs(NAME_DIR)
            ...
            with open(NAME, "r", encoding="utf-8") as f:
                ...
                if data == {}:
                    data.update({0: [[metric['bbox_mAP'], metric['bbox_mAP_50'], metric['bbox_mAP_75'], metric['bbox_mAP_s'], metric['bbox_mAP_m'], metric['bbox_mAP_l']], least_cost, \
                        least_iou_cost_final, result_ours, least_reg_cost_final, least_cls_cost_final, num_cls_large_k, mean_cls_large_k, results_cls_entropy_small_k]})
                else:
