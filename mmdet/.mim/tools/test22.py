...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            pseudo_ious2.append(((model_score>0.2).sum()/remove_score.shape[0]).item())
            ...
            # pseudo_ious2.append(0)  # 이거 안 하는 게 나음 /한 경우 결과 res_ours1_soft_add_no_adj0
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                conf1  = 1 / (1 + np.exp(200 * (conf - 0.5)))
                pseudo_ious1 = 1 / (1 + np.exp(-200 * (pseudo_ious - 0.5)))
                ...
                conf2  = 1 / (1 + np.exp(200 * (conf - 0.7)))
                pseudo_ious1_2 = 1 / (1 + np.exp(-200 * (pseudo_ious - 0.5)))

                count_soft = conf2 * pseudo_ious1_2
                if count_soft.sum()/keep.sum() > 0:
                    over_under_4.append(count_soft.sum()/keep.sum())


                count_soft = (conf2**0.6) * (pseudo_ious1_2**0.4)
                if count_soft.sum()/keep.sum() > 0:
                    over_under_5.append(count_soft.sum()/keep.sum())

                count_soft = (conf2**0.4) * (pseudo_ious1_2**0.6)
                if count_soft.sum()/keep.sum() > 0:
                    over_under_6.append(count_soft.sum()/keep.sum())

                conf3  = 1 / (1 + np.exp(200 * (conf - 0.7)))
                pseudo_ious1_3 = 1 / (1 + np.exp(-200 * (pseudo_ious - 0.7)))

                count_soft = conf3 * pseudo_ious1_3
                if count_soft.sum()/keep.sum() > 0:
                    over_under_7.append(count_soft.sum()/keep.sum())


                count_soft = (conf3**0.6) * (pseudo_ious1_3**0.4)
                if count_soft.sum()/keep.sum() > 0:
                    over_under_8.append(count_soft.sum()/keep.sum())

                count_soft = (conf3**0.4) * (pseudo_ious1_3**0.6)
                if count_soft.sum()/keep.sum() > 0:
                    over_under_9.append(count_soft.sum()/keep.sum())

                # res_ours1_soft_vary_center_k100부터는 over_under_10~12 추가 , k=100, 75
                conf  = 1 / (1 + np.exp(200 * (conf - 0.5)))
                pseudo_ious = 1 / (1 + np.exp(-200 * (pseudo_ious - 0.5)))

                count_soft = conf * pseudo_ious
                if (count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2.float())) > 0:
                    over_under_10.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2.float()))

                count_soft = (conf**0.6) * (pseudo_ious**0.4)
                if (count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2.float())) > 0:
                    over_under_11.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2.float()))

                count_soft = (conf**0.4) * (pseudo_ious**0.6)
                if (count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2.float())) > 0:
                    over_under_12.append(count_soft.sum()/keep.sum()+0.1*torch.mean(pseudo_ious2.float()))
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            # NAME_DIR  = "./res_ours1_soft_add_no_adj0/" + str(args.tag1)
            NAME_DIR  = "./res_ours1_soft_k200/" + str(args.tag1)
