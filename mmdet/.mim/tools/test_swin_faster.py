...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            # model_score1 = abs(abs(predicted_score-remove_score)-(1-pre_iou.squeeze(0)))
            model_score1 = abs(predicted_score-remove_score)-(1-pre_iou.squeeze(0))
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                # # under = (0.5 + 0.5 / (1 + np.exp(-10 * (score_all - 0.7)))).sum()  #predicted score는 빼서 계산해보기
                # # under = adj_box_num.sum()  # 1
                # under = (0.5 + 0.5 / (1 + np.exp(-10 * (score_all[~keep] - 0.7)))).sum()  #2
                # # print(under)
                # # print(adj_box_num.sum())
                # # exit()
                ...
                conf_up = conf > 0.5
                under = (0.5 + 0.5 / (1 + np.exp(-10 * (score_all[~keep] - 0.5)))).sum() 
                num_box_ratio = adj_box_num[conf_up].sum()/under
                if under.item() > 0 :
                    over_under_6.append(num_box_ratio.item())
                # print(num_box_ratio)  
                # exit()
                # print(num_box_ratio.item())
                # exit()
                # over_under_6.append(num_box_ratio.item())
                # if under.item() > 0 :
                #     over_under_6.append(num_box_ratio.item())
                
                # print(pseudo_ious.shape)
                # print(conf.shape)
                # exit()
                ...
                # # pseudo_ious2 = 1 / (1 + np.exp(-10 * (pseudo_ious2 - 0.5))) # car_swin_faster_rcnn
                # pseudo_ious2 = 1 / (1 + np.exp(-100 * (pseudo_ious2 - 0.5))) #car_swin_faster_rcnn2
                # com_iou = pseudo_ious
                ...
                # count_ori2 = (conf_ori+com_iou5)/2
                ...
                # print(num_box_ratio/(count_soft.sum()/keep.sum()))
                # exit()
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./02.23/ablation_conf_thr0.5_swin_faster/" + str(args.tag1)
