...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            if inter_over_pseudo_box.item() >= 0.75 :
                ...
                # # print("################################################################3")
                pseudo_iou_75 = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox.unsqueeze(0))
                # print(pseudo_iou)
                pseudo_ious.append(pseudo_iou_75.item())
            else:
                pseudo_iou = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox.unsqueeze(0))
                pseudo_ious.append(pseudo_iou.item())
            if inter_over_pseudo_box.item() >= 0.7 :    
                # ######ori#####
                x1_stan, y1_stan, x2_stan, y2_stan = init_box[current_idx] 
                width = x2_stan-x1_stan
                height = y2_stan-y1_stan
                ...
                pseudo_bbox = torch.tensor([new_x1, new_y1, new_x2, new_y2])
                # # print("################################################################3")
                pseudo_iou_7 = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox.unsqueeze(0))
                pseudo_ious2.append(pseudo_iou_7.item())
            else:
...
def main():
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_resize_0.75_0.7/" + str(args.tag1)
