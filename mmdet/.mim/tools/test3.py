...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        agnostic_nms = (0.5 + 0.5 / (1 + np.exp(-10 * (remove_score - 0.8)))).sum()
        # agnostic_nms = (1 / (1 + np.exp(-10 * (remove_score - 0.8)))).sum()
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                ############basic
                conf_up = conf > 0.8
                under = (0.5 + 0.5 / (1 + np.exp(-10 * (score_all[~keep] - 0.8)))).sum() 
                # under = (1 / (1 + np.exp(-10 * (score_all[~keep] - 0.8)))).sum()
                num_box_ratio = adj_box_num[conf_up].sum()/under
                if under.item() > 0 :
                    over_under_6.append(num_box_ratio.item())
                ...
                conf1 = conf
                # conf1  = 1 / (1 + np.exp(100 * (conf1 - 0.5)))
                conf1  = 1 / (1 + np.exp(100 * (conf1 - 0.8)))
...
# 02.21/r50_retina_consistency_no_conf/
            ...
            NAME_DIR  = "./02.23_ori_re/ablation_conf_thr0.8/" + str(args.tag1)
