...
def apply_iou_filter(scores: torch.Tensor,
    ...
    return keep, torch.tensor(pseudo_ious), torch.tensor(pseudo_ious2), torch.tensor(reg), torch.tensor(reg2)
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep, pseudo_ious, pseudo_ious2, reg, reg2 = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                # reg 9
                # count_soft = conf2 * com_iou
                # count_soft2 = conf2 * com_iou2
                # count_soft3 = conf1 * com_iou4
                # count_soft4 = conf1 * com_iou5
                # count_soft5 = conf2 * com_iou4
                # count_soft6 = conf2 * com_iou5
                # count_soft7 = conf3 * com_iou4
                # count_soft8 = conf3 * com_iou5
                # count_soft9 = conf4 * com_iou4
                # count_soft10 = conf4 * com_iou5

                count_soft = com_iou / conf2
                count_soft2 = com_iou2/conf2 
                count_soft3 = com_iou4/conf1
                count_soft4 = com_iou5/conf1 
                count_soft5 = com_iou4/conf2 
                count_soft6 = com_iou5/conf2 
                count_soft7 = com_iou4/conf3 
                count_soft8 = com_iou5/conf3 
                count_soft9 = com_iou4/conf4 
                count_soft10 = com_iou5/conf4 
                ...
                if count_soft.sum() > 0:
                    over_under.append(count_soft.sum())

                if count_soft2.sum() > 0:
                    over_under_2.append(count_soft2.sum())

                if count_soft3.sum() > 0:
                    over_under_3.append(count_soft3.sum())

                if count_soft4.sum() > 0:
                    over_under_4.append(count_soft4.sum())

                if count_soft5.sum() > 0:
                    over_under_5.append(count_soft5.sum())

                if count_soft6.sum() > 0:
                    over_under_6.append(count_soft6.sum())
                if count_soft7.sum() > 0:
                    over_under_7.append(count_soft7.sum())
                if count_soft8.sum() > 0:
                    over_under_8.append(count_soft8.sum())
                if count_soft9.sum() > 0:
                    over_under_9.append(count_soft9.sum())
                if count_soft10.sum() > 0:
                    over_under_10.append(count_soft10.sum())
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./01.21/01.19/res_ours1_revise_no_resize_add_reg12/" + str(args.tag1)
