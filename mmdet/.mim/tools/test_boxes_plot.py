...
def draw_boxes(image, green_box=None, yellow_boxes=None, red_box=None, orange_box=None):
    # 빈 이미지 생성
    # image = Image.new('RGB', image_size, color='white')
    draw = ImageDraw.Draw(image)

    # 노란색 박스 그리기
    if yellow_boxes is not None:
        yellow_boxes = tensor_to_list(yellow_boxes)
        for box in yellow_boxes:
            draw.rectangle(box, outline='yellow', width=1)

    # 빨간색 박스 그리기 (노란색 박스를 모두 포함하는 가장 작은 박스)
    if red_box is not None:
        red_box = tensor_to_list(red_box)
        draw.rectangle(red_box, outline='red', width=1)

    # 초록색 박스 그리기
    if green_box is not None:
        green_box = tensor_to_list(green_box)
        draw.rectangle(green_box, outline='green', width=1)

    if orange_box is not None:
        orange_box = tensor_to_list(orange_box)
        draw.rectangle(orange_box, outline='orange', width=1)
...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            # x1, y1, x2, y2 = torch.split(removed_box, 1, dim=1)
            # # 중심 좌표 계산
            # cx = (x1 + x2) / 2
            # cy = (y1 + y2) / 2
            # half_width = width / 2
            ...
            # resize_removed_box = torch.cat((new_x1, new_y1, new_x2, new_y2), dim=1)

            # x1_min = torch.min(resize_removed_box[:, 0])
            # y1_min = torch.min(resize_removed_box[:, 1])
            
            # # 각 bbox의 우하단 좌표 (x2, y2)
            # x2_max = torch.max(resize_removed_box[:, 2])
            # y2_max = torch.max(resize_removed_box[:, 3])
            ...
            mask = torch.triu(torch.ones(removed_iou.size(0), removed_iou.size(0)), diagonal=1).bool()
            # 대각 성분 제외한 값들만 필터링
            non_diag_values = removed_iou[~diag_mask]
            non_diag_values2 = removed_iou[mask]
            # 평균 계산
            iou_mean = torch.mean(non_diag_values)
            # print(iou_mean)
            # iou_mean = torch.mean(non_diag_values2)  # 위에꺼랑 똑같음
            # print(iou_mean)


            centers = torch.zeros_like(removed_box)
            centers[:, 0] = (removed_box[:, 0] + removed_box[:, 2]) / 2  # x 중심
            centers[:, 1] = (removed_box[:, 1] + removed_box[:, 3]) / 2  # y 중심

            # 모든 중심점 쌍 사이의 거리 계산
            num_boxes = centers.size(0)
            distances = torch.cdist(centers[:, :2], centers[:, :2])
            # exit()
            # 대각 성분(자기 자신과의 거리)을 제외
            mask = torch.triu(torch.ones(num_boxes, num_boxes), diagonal=1).bool()
            non_diag_distances = distances[mask]
            # 평균 거리 계산
            distance_mean = torch.mean(non_diag_distances)
            x1_stan, y1_stan, x2_stan, y2_stan = init_box[current_idx] 
            width = x2_stan-x1_stan
            height = y2_stan-y1_stan
            ref_length = max(width, height)
            ref_length1 = ref_length/4
            ref_length2 = ref_length/3
            
            # if distance_mean.item() >= ref_length1.item():  # -0.94/-0.82
                # print(ref_length)
                # print(distance_mean)
                # print("#########################################")
            # print(distance_mean)
            ...
            # image = draw_boxes(image, green_box=ref_box, yellow_boxes=removed_box, red_box=pseudo_bbox)
            # image = draw_boxes(image, green_box=ref_box, red_box=pseudo_bbox)
            # image = draw_boxes(image, green_box=ref_box, yellow_boxes=removed_box, red_box=pseudo_bbox)
            if iou_mean.item() < 0.65:
                ...
                image = draw_boxes(image, orange_box=pseudo_bbox)
            ...
            # 하나의 외접 bbox 반환
            pseudo_bbox = torch.tensor([x1_min, y1_min, x2_max, y2_max])
            if distance_mean.item() >= ref_length2.item():   # -0.884/-0.879
                x1_stan, y1_stan, x2_stan, y2_stan = init_box[current_idx] 
                width = x2_stan-x1_stan
                height = y2_stan-y1_stan
                x1, y1, x2, y2 = pseudo_bbox
            ...
            # image = draw_boxes(image, green_box=ref_box)
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                if score_up.sum()/(keep.sum()-score_down.sum()) > 0:
                    over_under_2.append(score_up.sum()/(keep.sum()-score_down.sum()))
                if (score_up_down.sum()+score_down_up.sum())/keep.sum() > 0:
                    over_under_3.append((score_up_down.sum()+score_down_up.sum())/keep.sum())
                if (score_up_down.sum()+score_down_up.sum())/(keep.sum()-score_down.sum()) > 0:
                    over_under_4.append((score_up_down.sum()+score_down_up.sum())/(keep.sum()-score_down.sum()))
                #####################################

                ############ res_1007_2 ####################
                if (score_down.sum()/keep.sum()) > 0:
                    over_under_5.append(score_down.sum()/keep.sum())
                if (score_up_down.sum()/keep.sum()) > 0 :
                    over_under_6.append(score_up_down.sum()/keep.sum())
                if (score_down_up.sum()/keep.sum()) > 0 :
                    over_under_7.append(score_down_up.sum()/keep.sum())
                if (1-(score_down_up.sum()/keep.sum())) > 0 :
                    over_under_8.append(1-(score_down_up.sum()/keep.sum()))
                if (score_down_up2.sum()/keep.sum()) > 0 :
                    over_under_9.append(score_down_up2.sum()/keep.sum())
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_split_merge_before_resizing2/" + str(args.tag1)
