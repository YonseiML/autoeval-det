...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            iou_cost = iou_loss(init_box[current_idx].unsqueeze(0) , removed_box)
            iou_matched_row_inds, iou_matched_col_inds = linear_sum_assignment(iou_cost)
            least_iou_cost_final = iou_cost[iou_matched_row_inds, iou_matched_col_inds].sum().numpy().tolist()
            ioucost.append(-least_iou_cost_final)
            # print(-least_iou_cost_final)
            # exit()
            ...
            # iou_cost = iou_loss(init_box[current_idx].unsqueeze(0) , pseudo_bbox.unsqueeze(0))
            # iou_matched_row_inds, iou_matched_col_inds = linear_sum_assignment(iou_cost)
            # least_iou_cost_final = iou_cost[iou_matched_row_inds, iou_matched_col_inds].sum().numpy().tolist() 
            # exit()
            # ioucost.append(-least_iou_cost_final)
    ...
    return keep, torch.tensor(ioucost)
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        # exit()  
        # print(num_flag[area_idx]) ## 250
        # print(len(over_under))  ##204
        # print(len(over_under_7))  ## 250
        # exit() 
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_giou/" + str(args.tag1)
