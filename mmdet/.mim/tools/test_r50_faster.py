...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            # model_score1 = abs(abs(predicted_score-remove_score)-(1-pre_iou.squeeze(0))) #sr3
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                # # pseudo_ious2 = 1 / (1 + np.exp(-10 * (pseudo_ious2 - 0.5))) # ori 얘가 더 좋음
                # pseudo_ious2 = 1 / (1 + np.exp(-10 * (pseudo_ious2 - 0.6))) # 3 ori보다 좋음 
                # # pseudo_ious2 = 1 / (1 + np.exp(-100 * (pseudo_ious2 - 0.5)))  # 2
                # com_iou = pseudo_ious
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./02.21/r50_faster_CCC/" + str(args.tag1)
