...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            reg2.append(torch.mean(model_score).item())
            # exit()
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                # exit()
                ### 01.18부터 코드 변경
                conf2 = conf-0.05*torch.mean(reg)
                conf2  = 1 / (1 + np.exp(100 * (conf2 - 0.5)))
                conf1  = 1 / (1 + np.exp(100 * (conf - 0.5)))
                conf3 = conf-0.5*torch.mean(reg)
                ...
                count_soft6 = (conf2+com_iou6)/2
                count_soft7 = (conf2+com_iou7)/2
                count_soft8 = (conf1+com_iou5)/2
                # count_soft9 = (conf1+com_iou7)/2  # ## s0.1, s0.25일 때는 주석처러한게 over_under_12임 comb3_s0.1은 그 밑에꺼 적용용
                count_soft9 = (conf3+com_iou5)/2
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./01.21/01.18/res_ours1_revise_no_resize_CCC_S0.1_comb3+reg_all/" + str(args.tag1)
