...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            # print(abs(abs(predicted_score-remove_score)-(1-pre_iou)))
            # print(pre_iou.shape)
            model_score = abs(abs(predicted_score-remove_score)-(1-pre_iou.squeeze(0)))
            # print(model_score>0.2)
            # print((abs(predicted_score-remove_score))[model_score > 0.2])
            # print((1-pre_iou.squeeze(0))[model_score > 0.2])
            # print((model_score>0.2).sum())
            # print(remove_score.shape[0])
            # print((model_score>0.2).sum()/remove_score.shape[0])
            model_scores.append(((model_score>0.2).sum()/remove_score.shape[0]).item())
            # exit()
            # print(abs(predicted_score-remove_score)/(1-pre_iou))
            # model_score = abs(predicted_score-remove_score)/(1-pre_iou)
            # model_score = torch.mean(model_score)
            # model_scores.append(model_score.item())
            # print(model_score)
            # exit()
            # wbf
            labels_list = [[0], [0]]
            scores_list = [[predicted_score], [predicted_score]] # 우선 동일하게
            x1, y1, x2, y2 = predicted_bbox 
            x1_min, y1_min, x2_max, y2_max = pseudo_bbox
            # print(img_h, img_w)  #800 1333
            # exit()
            if y2_max > 800:
                img_h = y2_max
            if x2_max > 1333:
                img_w = x2_max
            # if ((y2_max > 800) or (x2_max > 1333)) and ((y2_max<=800*1.5) or (x2_max<=1333*1.5)):
            #     img_h, img_w = img_h*1.5, img_w*1.5
            # # elif (y2_max > 800*1.5) or (x2_max > 1333*1.5):
            # #     img_h, img_w = img_h*2, img_w*2
            # elif (y2_max > 800*2) or (x2_max > 1333*2):
            #     img_h, img_w = img_h*2.5, img_w*2.5
            boxes_list = [[[x1/img_w, y1/img_h, x2/img_w, y2/img_h]], [[x1_min/img_w, y1_min/img_h, x2_max/img_w, y2_max/img_h]]]
            # print(boxes_list)
            # print(x1/img_w, y1/img_h, x2/img_w, y2/img_h)
            # print(x1_min/img_w, y1_min/img_h, x2_max/img_w, y2_max/img_h)
            ...
            # weights = [2, 1] 를 달리 해서도 실험해보기 
            boxes, _, _ = weighted_boxes_fusion(
                boxes_list, 
                scores_list, 
                labels_list, 
                weights=None,
            ...
            # exit()
            # print(boxes)
            # pseudo_bbox = boxes.squeeze(0)
            pseudo_bbox = boxes[0]
            # print(pseudo_bbox)
            # exit()
            pseudo_bbox = torch.tensor(pseudo_bbox).cuda()
            # print(boxes)
            # exit()
            ...
            # ratio_adjacent = torch.max(area_ratio_p_a/distance_pre_adjacent)
            ...
            #     # print(pseudo_iou)
            # wbf version
            pre_bbox = torch.tensor([x1/img_w, y1/img_h, x2/img_w, y2/img_h]).cuda()
            pseudo_iou = torchvision.ops.box_iou(pre_bbox.unsqueeze(0) , pseudo_bbox.unsqueeze(0))
            # print(pseudo_iou.item())  # 0.6521
            # exit()
            pseudo_ious.append(pseudo_iou.item())
    ...
    return keep, torch.tensor(pseudo_ious), torch.tensor(pseudo_ious2), torch.tensor(model_scores)
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep, pseudo_ious, pseudo_ious2, model_score = apply_iou_filter(score_all, iou_all, 0.5, init_box, img_size)
                ...
                # print(model_score)
                # exit()
                over_under_8.append(torch.mean(model_score))
                print(torch.mean(model_score))
                exit()
                # print(pseudo_ious)
                # print(conf)
                # exit()
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_revise_stable/" + str(args.tag1)
