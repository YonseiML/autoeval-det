...
sys.path.append(os.getcwd())  # 모듈이 있는 상위 디렉토리 경로 추가
...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            pseudo_ious.append(torch.mean(distance).item())
            ...
            # x1 = torch.max(removed_box[:, 0], ref_bbox[:, 0])
            # y1 = torch.max(removed_box[:, 1], ref_bbox[:, 1])
            # x2 = torch.min(removed_box[:, 2], ref_bbox[:, 2])
            # y2 = torch.min(removed_box[:, 3], ref_bbox[:, 3])
            ...
            #     # 교집합 면적 / box1의 면적
            # ref_iou = intersection / ref_box_area[:, None]
            # # print(torch.mean(ref_iou))
            # pseudo_ious.append(torch.mean(ref_iou).item())
            # # exit()
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                #### resize.py 추출하는 코드 ####
                # keep, pseudo_ious, pseudo_ious2, change = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                conf_up = conf > 0.5
                conf_down = conf <= 0.5
                iou_up = pseudo_ious > 5
                iou_down = pseudo_ious <= 5
                ...
                iou_up2 = pseudo_ious > 6
                iou_down2 = pseudo_ious <= 6
                ...
                iou_up3 = pseudo_ious > 7
                iou_down3 = pseudo_ious <= 7

                score_up3 = conf_up & iou_up3
                score_up_down3 = conf_up & iou_down3
                score_down_up3 = conf_down & iou_up3
                score_down3 = conf_down & iou_down3


                iou_up4 = pseudo_ious > 8
                iou_down4 = pseudo_ious <= 8

                score_up4 = conf_up & iou_up4
                score_up_down4 = conf_up & iou_down4
                score_down_up4 = conf_down & iou_up4
                score_down4 = conf_down & iou_down4


                iou_up5 = pseudo_ious > 9
                iou_down5 = pseudo_ious <= 9

                score_up5 = conf_up & iou_up5
                score_up_down5 = conf_up & iou_down5
                score_down_up5 = conf_down & iou_up5
                score_down5 = conf_down & iou_down5
                ...
                if (score_down.sum()/keep.sum()) > 0 :
                    over_under.append(score_down.sum()/keep.sum())
                if (score_down2.sum()/keep.sum()) > 0 :
                    over_under_2.append(score_down2.sum()/keep.sum())
                if (score_down3.sum()/keep.sum()) > 0 :
                    over_under_3.append(score_down3.sum()/keep.sum())
                if (score_down4.sum()/keep.sum()) > 0 :
                    over_under_4.append(score_down4.sum()/keep.sum())
                if (score_down5.sum()/keep.sum()) > 0 :
                    over_under_5.append(score_down5.sum()/keep.sum())
            #####################################################
        ...
        # # list1 = f_inter_1s_b_value
        # # list2 = f_inter_1s_a_value
        ...
        # # plt.scatter(x_values, list1, color='black', label='area(intersection)==1')
        ...
        # # plt.scatter(x_values, list2, color='black', label='area(intersection)!=1')
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_study/" + str(args.tag1)
