...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                conf_up = conf > 0.5
                conf_down = conf <= 0.5
                conf_up_iou = pseudo_ious[conf_up]
                conf_down_iou = pseudo_ious[conf_down]
                conf_up = conf[conf_up]
                conf_down = conf[conf_down]
                # print(pseudo_ious)
                # print(conf_up_iou)
                # print(conf_down_iou)
                # print((conf_down_iou > 1).sum()/keep.sum())
                score_down_up = conf_down_iou > 1
                # exit()
                # if (score_down_up.sum()/keep.sum()) > 0 :
                #     over_under.append(score_down_up.sum()/keep.sum())

                if torch.mean(conf_up_iou) > 0 :
                    over_under.append(torch.mean(conf_up_iou))
                if torch.mean(conf_down_iou) > 0 :
                    over_under_2.append(torch.mean(conf_down_iou))
                if torch.sum(conf*pseudo_ious) > 0:
                    over_under_3.append(torch.sum(conf*pseudo_ious))
                if torch.mean(conf*pseudo_ious) > 0:
                    over_under_4.append(torch.mean(conf*pseudo_ious))    # 이거 높음
                if torch.sum(conf_down_iou)/torch.sum(pseudo_ious) > 0:
                    over_under_5.append(torch.sum(conf_down_iou)/torch.sum(pseudo_ious))
                if torch.sum(conf_up_iou)/torch.sum(pseudo_ious) > 0:
                    over_under_6.append(torch.sum(conf_up_iou)/torch.sum(pseudo_ious))
                if torch.mean(conf_down_iou)/torch.mean(pseudo_ious) > 0:
                    over_under_7.append(torch.mean(conf_down_iou)/torch.mean(pseudo_ious))
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_ours1_revise_no_resize3/" + str(args.tag1)
