...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        sorted_remove = remove_score.sort(descending=True).indices
        # removed_box = removed_box[sorted_remove[:4]]
        removed_box = removed_box[remove_score >= 0.5]
        # print(remove_score)
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            # print(torch.mean(pseudo_iou, dim=-1))
            # print(pseudo_iou)
            adj = pseudo_iou >= 0.85 
...
def main():
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_revise_agnostic_nms_pre/" + str(args.tag1)
