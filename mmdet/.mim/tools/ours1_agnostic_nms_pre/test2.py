...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        # removed_box = removed_box[remove_score > predicted_score]
        # print(remove_score)
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            # 새로운 bbox
            # print(new_x1.shape)
            removed_box = torch.cat([new_x1.unsqueeze(-1), new_y1.unsqueeze(-1), new_x2.unsqueeze(-1), new_y2.unsqueeze(-1)], dim=-1)
            # removed_box = torch.tensor([new_x1, new_y1, new_x2, new_y2])

            pseudo_iou = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , removed_box)
            adj = pseudo_iou >= 0.85
            # print(pseudo_iou.shape[-1])
            # print(adj.sum()/pseudo_iou.shape[-1])
            pseudo_ious2.append((adj.sum()/pseudo_iou.shape[-1]).item())
            pseudo_ious.append(torch.mean(pseudo_iou, dim=-1).item())
            # print(removed_box.shape)
            # exit()
            ...
            # if ratio_merged.item() > 1.5 * ratio_adjacent.item():
            # # if area_ratio_p_m.item() > torch.max(area_ratio_p_a).item():
            #     # yes += 1
            #     if removed_box.shape[0] > 1 :
            #         _, max_area = torch.max(removed_box_area, dim=0)
            #         x1_stan, y1_stan, x2_stan, y2_stan = removed_box[max_area]
            #     else:
            #         x1_stan, y1_stan, x2_stan, y2_stan = removed_box
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                # exit()
                iou_up = pseudo_ious > 0.85
                iou_down = pseudo_ious <= 0.85
                iou_up2 = pseudo_ious > 0.5
                iou_down2 = pseudo_ious <= 0.5
                ...
                if (score_down.sum()/keep.sum()) > 0 :
                    over_under_2.append(score_down.sum()/keep.sum())
                ...
                if (score_down2.sum()/keep.sum()) > 0 :
                    over_under_5.append(score_down2.sum()/keep.sum())
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_revise2/" + str(args.tag1)
