...
def apply_iou_filter(scores: torch.Tensor,
    ...
    return keep, torch.tensor(pseudo_ious)
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep2, pseudo_ious2 = apply_iou_filter2(score_all, iou_all, 0.5, init_box)
                ...
                if (score_down_up.sum()/(score_up.sum()+score_down_up.sum())) > 0 :
                    over_under_7.append(score_down_up.sum()/(score_up.sum()+score_down_up.sum()))
                if (score_down_up2.sum()/(score_up2.sum()+score_down_up2.sum())) > 0 :
                    over_under_8.append(score_down_up2.sum()/(score_up2.sum()+score_down_up2.sum()))              
            #####################################################
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_1014_consistency_whole/" + str(args.tag1)
            ...
            with open(NAME, "r", encoding="utf-8") as f:
                ...
                if data == {}:
                    data.update({0: [[metric['bbox_mAP'], metric['bbox_mAP_50'], metric['bbox_mAP_75'], metric['bbox_mAP_s'], metric['bbox_mAP_m'], metric['bbox_mAP_l']], least_cost, \
                        least_iou_cost_final,  result_over_under, result_over_under_2, result_over_under_3, result_over_under_4, result_over_under_5, result_over_under_6, result_over_under_7, result_over_under_8, least_reg_cost_final, least_cls_cost_final, num_cls_large_k, mean_cls_large_k, results_cls_entropy_small_k]})
                else:
