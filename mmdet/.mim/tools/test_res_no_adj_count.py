...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            if iou_mean.item() < 0.7:   # 0.65이 베스트 
    ...
    return keep, torch.tensor(pseudo_ious), torch.tensor(pseudo_ious2), no, no2
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                keep, pseudo_ious, pseudo_ious2, no, no2 = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                # print(no)
                # exit()
                if ((score_down_up2.sum()+no2)/keep.sum()) > 0 :
                    over_under.append((score_down_up2.sum()+no2)/keep.sum())
                if (score_down_up2.sum()+score_up2.sum()+no2)/keep.sum() > 0:
                    over_under_2.append((score_down_up2.sum()+score_up2.sum()+no2)/keep.sum())
                if (score_down_up2.sum()+score_down2.sum()+no2)/keep.sum() > 0:
                    over_under_3.append((score_down_up2.sum()+score_down2.sum()+no2)/keep.sum())

                if ((score_down_up2.sum()+no)/keep.sum()) > 0 :
                    over_under_4.append((score_down_up2.sum()+no)/keep.sum())
                if (score_down_up2.sum()+score_up2.sum()+no)/keep.sum() > 0:
                    over_under_5.append((score_down_up2.sum()+score_up2.sum()+no)/keep.sum())
                if (score_down_up2.sum()+score_down2.sum()+no)/keep.sum() > 0:
                    over_under_6.append((score_down_up2.sum()+score_down2.sum()+no)/keep.sum())            
            #####################################################
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_no_adj_count/" + str(args.tag1)
