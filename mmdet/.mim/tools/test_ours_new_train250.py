...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default= 'overcast',
        help='which domain expert? e.g. overcast, night, wet, snow')
    parser.add_argument(
        '--domain',
        type=str,
        default= 'overcast',
        help='Data domain. e.g. overcast, night, wet, snow')
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            # print(score_out_ex.shape) #21
            # print(len(iou_ex))  #21
            # print(iou_out_ex.shape)  #torch.Size([21, 21])     
            # print(score_all.shape) #1000
            # print(score_ex.shape)  # 21
            # print(outputs[7][img_idx][0].shape)  #(274,)
            # print(outputs[8][img_idx][0].shape)   #(21,) 
            score_all = torch.tensor(results_cls_per_img_ori[img_idx]) #1000
            iou_all = iou_alls[img_idx] # 249
            init_box = torch.tensor(np.array(init_boxes[img_idx]))
        
            if len(iou_all) > 0 :

                ####이 세개가 다 같게 나와야함#########
                # print(outputs[7][img_idx][0].shape)  #(21,)
                # print(len(iou_all))  # (274,)
                # print(score_all.shape)  # 274
                iou_all = torch.tensor(np.array(iou_all)) #249,249
                # print(iou_all.shape)  # torch.Size([274, 274])
                first_filter = score_all>0.05 # 1000 -> 249 / zeros
                # print(score_all)
                # exit()
                score_all = score_all[first_filter.bool()] #249
                # iou_all = iou_all[first_filter.bool(),:][:, first_filter.bool()]
                # init_box = init_box[first_filter.bool(),:] #249,4 at 0
                ...
                if score_down_up.sum()/keep.sum() > 0:
                    over_under.append(score_down_up.sum()/keep.sum()) # 여기서 over_under에 들어가는 값을 다른거로 바꿔봐야할듯...
                ...
                if score_up.sum()/keep.sum() > 0:
                    over_under_4.append(score_up.sum()/keep.sum())
                if score_up_down.sum()/keep.sum() > 0:
                    over_under_5.append(score_up_down.sum()/keep.sum())
                if score_down.sum()/keep.sum() > 0:
                    over_under_6.append(score_down.sum()/keep.sum())
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = f"./res_multiweather_train_250_model/{args.expert}_expert/{str(args.tag1)}/" 
