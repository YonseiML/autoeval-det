...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            ref_remove_iou = iou_values_all[current_idx][iou_with_remaining]
            iou_ref_mean = torch.mean(ref_remove_iou)
           
            iou_mean = torch.mean(removed_iou)
            # print((iou_mean.item() < 0.7) & (iou_ref_mean.item() > 0.8))
            # exit()
            if (iou_mean.item() < 0.7) & (iou_ref_mean.item() > 0.6) :   # 원래는 0.8
                # modi += 1
                # print(remove_score)
...
def main():
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                if (score_up.sum()+score_down_up.sum())/keep.sum() > 0 :
                    over_under_6.append((score_up.sum()+score_down_up.sum())/keep.sum())
                if (score_down_up.sum()+score_down.sum())/keep.sum() > 0 :
                    over_under_7.append((score_down_up.sum()+score_down.sum())/keep.sum())       
                if (score_up_down.sum()+score_down_up.sum())/keep.sum() > 0 :
                    over_under_8.append((score_up_down.sum()+score_down_up.sum())/keep.sum())      
            #####################################################
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_split_resize_ref_iou_0.6/" + str(args.tag1)
