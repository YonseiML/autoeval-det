...
def apply_iou_filter(scores: torch.Tensor,
    ...
    for i in range(num_boxes):
        ...
        if removed_box.numel() != 0:  # # removed_box가 비어 있지 않을 때 처리
            ...
            # inter_over_pseudo_box = intersection / pseudo_box_area[:, None]
            ...
            # if iou_mean.item() < 0.65:   # 0.65이 베스트 0.7
                # print(x2_max-x1_min)
                # print(y2_max-y1_min)
            pseudo_width = x2_max-x1_min
            pseudo_height = y2_max-y1_min
            ref_bbox = ref_bbox.squeeze(0)
            x1_min, y1_min, x2_max, y2_max = ref_bbox
            # print(x2_max-x1_min)
            # print(y2_max-y1_min)
            ref_width = x2_max-x1_min
            ref_height = y2_max-y1_min
            pseudo_area = pseudo_width * pseudo_height
            ref_area = ref_width*ref_height
            # print(pseudo_width/ref_width)
            # print(pseudo_height/ref_height)
            if (iou_mean.item() >= 0.65) :
                pseudo_iou = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox.unsqueeze(0))
                iou_no_resize.append(pseudo_iou.item())
            # if (pseudo_width/ref_width >= 1) & (pseudo_height/ref_height >=1):
            if (iou_mean.item() < 0.65) :
                area_yes_resize.append((pseudo_area/ref_area).item())
                inter.append(inter_over_pseudo_box.item())   
            # if (pseudo_width/ref_width >= 1) & (pseudo_height/ref_height >=1) & (iou_mean.item() < 0.65):  # 이걸로 res_upgrade
            # if (pseudo_width/ref_width >= 1) & (pseudo_height/ref_height >=1) & (iou_mean.item() < 0.65) & (inter_over_pseudo_box==1):
            # if (iou_mean.item() < 0.65) & (inter_over_pseudo_box==1):  # 위의 조건식이랑 똑같음
                pseudo_iou = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox.unsqueeze(0))
                iou.append(pseudo_iou.item())
                iou_yes_resize.append(pseudo_iou.item())
                before = pseudo_iou.clone()
                # print(pseudo_iou)
                if inter_over_pseudo_box==1:
                    before_1.append(pseudo_iou.item())
                else:
                    before_not1.append(pseudo_iou.item())
                # print("#######################################333")
                # yes += 1  # 74: ratio&조건일 때 , 49: iou 0.65일 때, 47: 두 모든 조건 
            # if inter_over_pseudo_box.item() >= 0.8 :
            # if (iou_mean.item() < 0.65) & (inter_over_pseudo_box.item() == 1) :
            #     pseudo_bbox = pseudo_bbox.unsqueeze(0)
            #     x1 = torch.max(pseudo_bbox[:, None, 0], removed_box[:, 0])
            #     y1 = torch.max(pseudo_bbox[:, None, 1], removed_box[:, 1])
            #     x2 = torch.min(pseudo_bbox[:, None, 2], removed_box[:, 2])
            #     y2 = torch.min(pseudo_bbox[:, None, 3], removed_box[:, 3])
            #     inter_w = (x2 - x1).clamp(min=0)
            #     inter_h = (y2 - y1).clamp(min=0)
            ...
            #     print(intersection / pseudo_box_area[:, None])
            #     pseudo_bbox = pseudo_bbox.squeeze(0)
            # if (iou_mean.item() < 0.7) & (iou_mean2.item() < 0.7) & (inter_over_pseudo_box.item() >= 2/3):
                ...
                pseudo_iou = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox.unsqueeze(0))
                # print(pseudo_iou)
                after = pseudo_iou.clone()
                if inter_over_pseudo_box==1:
                    after_1.append(pseudo_iou.item())
                else:
                    after_not1.append(pseudo_iou.item())

            else:
                area_no_resize.append((pseudo_area/ref_area).item())
            pseudo_iou = torchvision.ops.box_iou(init_box[current_idx].unsqueeze(0) , pseudo_bbox.unsqueeze(0))
            pseudo_iou2 = pseudo_iou.clone()
            # print(pseudo_iou)
            # if (pseudo_width/ref_width >= 1) & (pseudo_height/ref_height >=1): 
            #     pseudo_ious2.append(1)
            # else:
            #     pseudo_ious2.append(pseudo_iou.item())
            if (iou_mean.item() < 0.65) :
                pseudo_iou = max(before, after)
                pseudo_iou2 = min(before, after)

            pseudo_ious2.append(pseudo_iou.item())
            pseudo_ious.append(pseudo_iou2.item())
    ...
    # print(before_not1)
    # print(after_not1)
    # print(area_yes_resize)
    # print(area_no_resize)
    # exit()
    iou_yes_no = torch.tensor(iou_yes_resize), torch.tensor(iou_no_resize)
    change = torch.tensor(before_1), torch.tensor(before_not1), torch.tensor(after_1), torch.tensor(after_not1)
    # return kept_indices, remove_bbox
    return keep, torch.tensor(pseudo_ious), torch.tensor(pseudo_ious2), change, torch.tensor(inter), torch.tensor(iou), torch.tensor(area_yes_resize), torch.tensor(area_no_resize), iou_yes_no
...
def main():
    ...
    for area_idx, area in enumerate(areaRng):
        ...
        for img_idx in range(len(results_bbox_per_img)):
            ...
            if len(iou_all) > 0 :
                ...
                #### resize.py 추출하는 코드 ####
                keep, pseudo_ious, pseudo_ious2, change, inter, iou, area_yes_resize, area_no_resize, iou_yes_no = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                ...
                before_1, before_not1, after_1, after_not1 = change
                iou_yes, iou_no = iou_yes_no
                
                iou_yess.append(iou_yes)
                iou_nos.append(iou_no)
                b_1 = before_1 <= 0.5
                b_not1 = before_not1 <= 0.5
                a_1 = after_1 <= 0.5
                a_not1 = after_not1 <= 0.5
                inter_1 = b_1 | a_1
                inter_not1 = b_not1 | a_not1
                
                inter_1s.append((inter_1.sum()/inter_1.shape[0]).item())
                inter_not1s.append((inter_not1.sum()/inter_not1.shape[0]).item())
                
                # area_yes_resizes.append(area_yes_resize)
                # area_no_resizes.append(area_no_resize)
                area_yes_resizes.append(torch.mean(area_yes_resize).item())
                area_no_resizes.append(torch.mean(area_no_resize).item())

                if (inter_1.sum()/inter_1.shape[0]).item() > 0:
                    inter_1s_b_value.append(before_1[inter_1.bool()])
                    inter_1s_a_value.append(after_1[inter_1.bool()])
                if (inter_not1.sum()/inter_not1.shape[0]).item() > 0:
                    inter_not1s_b_value.append(before_not1[inter_not1.bool()])
                    inter_not1s_a_value.append(after_not1[inter_not1.bool()])
                # keep, pseudo_ious, pseudo_ious2, change, inter, iou = apply_iou_filter(score_all, iou_all, 0.5, init_box)
                # conf = torch.tensor(results_cls_per_img[img_idx])
                # if conf.shape != pseudo_ious.shape:
                #     conf, _ = torch.sort(score_all[keep], descending=True)
                #     print("nnnnnnnnnnnnnnnnnnnnnnnnnnn")  # coco 49 에서만 4번 있음
                ...
                # exit()
                if (score_up2.sum()/keep.sum()) > 0 :
                    over_under.append(score_up2.sum()/keep.sum())
                if (score_up_down2.sum()/keep.sum()) > 0 :
                    over_under_2.append(score_up_down2.sum()/keep.sum())
                if (score_down2.sum()/keep.sum()) > 0 :
                    over_under_3.append(score_down2.sum()/keep.sum())    
                ...
                if (1-(score_down_up2.sum()/keep.sum())) > 0 :
                    over_under_7.append(1-(score_down_up2.sum()/keep.sum()))

                if (score_down_up.sum()/keep.sum()) > 0 :
                    over_under_8.append(score_down_up.sum()/keep.sum())
                if (score_up.sum()/keep.sum()) > 0 :
                    over_under_9.append(score_up.sum()/keep.sum())
                if (score_up_down.sum()/keep.sum()) > 0 :
                    over_under_10.append(score_up_down.sum()/keep.sum())
                if (score_down.sum()/keep.sum()) > 0 :
                    over_under_11.append(score_down.sum()/keep.sum()) 
            #####################################################
        ...
        # print("#############################################33")
        f_inter_1s_b_value = []
        for tensor in inter_1s_b_value:
            f_inter_1s_b_value.extend(tensor.tolist())
        # print(f_inter_1s_b_value)
        # print("#############################################33")
        f_inter_1s_a_value = []
        for tensor in inter_1s_a_value:
            f_inter_1s_a_value.extend(tensor.tolist())
        # print(f_inter_1s_a_value)
        # print("#############################################33")
        f_inter_not1s_b_value = []
        for tensor in inter_not1s_b_value:
            f_inter_not1s_b_value.extend(tensor.tolist())
        # print(f_inter_not1s_b_value)
        # print("#############################################33")
        f_inter_not1s_a_value = []
        for tensor in inter_not1s_a_value:
            f_inter_not1s_a_value.extend(tensor.tolist())
        # print(f_inter_not1s_a_value)
        f_inters =[]
        for tensor in inters:
            f_inters.extend(tensor.tolist())
        f_ious = []
        for tensor in ious:
            f_ious.extend(tensor.tolist())

        # f_area_yes_resize = []
        # for tensor in area_yes_resizes:
        #     f_area_yes_resize.extend(tensor.tolist())
        # f_area_no_resize = []
        # for tensor in area_no_resizes:
        #     f_area_no_resize.extend(tensor.tolist())
        
        f_iou_yes = []
        for tensor in iou_yess:
            f_iou_yes.extend(tensor.tolist())
        f_iou_no = []
        for tensor in iou_nos:
            f_iou_no.extend(tensor.tolist())
        # print(f_ious)
        # print(f_inters)
        ...
        # list1 = f_inter_1s_b_value[:100]
        # list2 = f_inter_1s_a_value[:100]

        # list1 = f_inter_not1s_b_value[:100]
        # list2 = f_inter_not1s_a_value[:100]

        # list1 = f_area_yes_resize
        # list2 = f_area_no_resize
        # list1 = area_yes_resizes
        # list2 = area_no_resizes
        # x축 값을 위한 인덱스 생성
        x_values = range(len(list1))
        # x_values = range(len(list2))
        
        # 그래프 그리기
        plt.figure(figsize=(8, 6))

        # List 1 산점도와 점선
        plt.scatter(x_values, list1, color='blue', label='resize O')
        # plt.scatter(x_values, list1, color='blue', label='before_resize')
        # plt.plot(x_values, list1, color='blue', linestyle='--')
        # plt.scatter(x_values, list1, color='black', label='adjacency==1')
        # plt.plot(x_values, list1, color='black', linestyle='--')
        # plt.scatter(x_values, list1, color='blue', label='iou')
        # plt.plot(x_values, list1, color='blue', linestyle='--')
        # # List 2 산점도와 점선
        # plt.scatter(x_values, list2, color='orange', label='resize X')
        # plt.scatter(x_values, list2, color='orange', label='after_resize')
        # plt.plot(x_values, list2, color='orange', linestyle='--')
        # plt.scatter(x_values, list2, color='black', label='adjacency!=1')
        # plt.plot(x_values, list2, color='black', linestyle='--')
        # plt.scatter(x_values, list2, color='orange', label='adjacency')
        # plt.plot(x_values, list2, color='orange', linestyle='--')

        # 그래프 제목과 축 레이블 설정
        # plt.title('IoU & adjacency')
        # plt.title('adjacency==1')
        # plt.title('before & after resizing')
        ...
        # legend만 저장
        plt.savefig('./IDEA/resize.pdf', bbox_inches='tight')
        exit()   
    ...
    if rank == 0:
        ...
        if len(outputs) >= 4:
            ...
            NAME_DIR  = "./res_resize_up_down/" + str(args.tag1)
